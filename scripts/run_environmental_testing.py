...
async def run_comprehensive_testing(site_info, base_date, output_dir):
    ...
    logger.info(
        f"🌊 Starting comprehensive environmental testing for {site_info['name']}"
    )
    ...
    results = await validator.run_comprehensive_testing(
        lat=site_info["lat"], lng=site_info["lng"], base_date=base_date
    )

    # Save results
    output_file = (
        Path(output_dir)
        / f"comprehensive_{site_info['name'].lower().replace(' ', '_')}_{base_date}.json"
    )
...
async def run_tidal_testing(site_info, base_date, output_dir):
    ...
    output_file = (
        Path(output_dir)
        / f"tidal_{site_info['name'].lower().replace(' ', '_')}_{base_date}.json"
    )
    save_results(results, output_file)
    ...
    if "summary" in results:
        ...
        if "detailed_results" in results:
            ...
            for result in results["detailed_results"]:
                ...
                print(
                    f"  {status} {result['condition_name']}: {result['detection_rate'] * 100:.1f}% detection"
                )
...
def print_overall_summary(all_results):
    ...
    for site_key, results in all_results.items():
        ...
        if "summary" in results:
            ...
            print(
                f"📍 {site_name}: {successful}/{tests} tests passed ({success_rate * 100:.1f}%)"
            )
