...
def demo_skema_validation():
    ...
    validation_sites = [
        ...
        {"name": "Haida Gwaii South", "skema_accuracy": 0.823, "our_accuracy": 0.871},
        {
    ...
    for site in validation_sites:
        print(f"📍 {site['name']}")
        print(
            f"   SKEMA: {site['skema_accuracy']:.1%} | Ours: {site['our_accuracy']:.1%}"
        )
    ...
    sites_better = sum(
        1 for s in validation_sites if s["our_accuracy"] > s["skema_accuracy"]
    )
    ...
    bars = axes[1].bar(
        range(len(formula_names)),
        equivalences,
        alpha=0.8,
        color=[
            "green" if e > 0.9 else "orange" if e > 0.8 else "red" for e in equivalences
        ],
    )
...
"""
    ...
    report_path = (
        output_dir / f"skema_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    )
