...
def run_skema_validation_demo():
    ...
    sites_better = sum(
        1 for r in benchmark_results if r["our_accuracy"] > r["skema_accuracy"]
    )
    ...
    for result in benchmark_results:
        ...
        print(
            f"| {result['site'][:20]:<20} | {result['skema_accuracy']:.1%} | {result['our_accuracy']:.1%} | {result['correlation']:.3f} | {result['rmse']:.3f} | {sig_marker} |"
        )
    ...
    print(
        f"• Statistically Significant Differences: {significant_sites}/{len(benchmark_results)}"
    )
    print()
    ...
    fig.suptitle(
        "SKEMA vs Our Pipeline: Processing Comparison", fontsize=14, fontweight="bold"
    )
...
"""
    ...
    data_path = (
        output_dir / f"validation_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
