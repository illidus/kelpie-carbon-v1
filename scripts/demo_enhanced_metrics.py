...
def generate_sample_validation_data():
    ...
    for i, coord in enumerate(coordinates):
        ...
        predicted_biomass = np.maximum(
            predicted_biomass, 0.05
        )  # Ensure positive values

        # Generate measurement dates
        start_date = datetime(2023, 1, 1)
        measurement_dates = [
            (start_date + timedelta(days=30 * j)).strftime("%Y-%m-%d")
        ...
        print(
            f"   ✅ {coord.name}: {n_measurements} measurements, species: {coord.species}"
        )
...
def demonstrate_individual_metrics():
    ...
    carbon_metrics = calculator.calculate_carbon_accuracy_metrics(
        predicted, observed, carbon_factors
    )
    ...
    print(
        f"      • Sequestration RMSE: {carbon_metrics['sequestration_rate_rmse_tc_year']:.4f} tC/year"
    )
...
def demonstrate_four_coordinate_validation():
    ...
    for site_name, result in validation_results.items():
        ...
        print(
            f"      🌱 Carbon R²:  {carbon_r2:.3f}, RMSE: {carbon_rmse:.3f} tC/hectare"
        )
...
def demonstrate_validation_summary():
    ...
    if "validation_summary" in summary:
        ...
        print(
            f"      📈 Mean biomass R²: {overall_biomass['mean_r2']:.3f} (±{overall_biomass['std_r2']:.3f})"
        )
        print(
            f"      📈 Mean biomass RMSE: {overall_biomass['mean_rmse_kg_m2']:.3f} kg/m²"
        )
        print(
            f"      🌱 Mean carbon R²: {overall_carbon['mean_r2']:.3f} (±{overall_carbon['std_r2']:.3f})"
        )
        print(
            f"      🌱 Mean carbon RMSE: {overall_carbon['mean_rmse_tc_hectare']:.3f} tC/hectare"
        )

        print("\n   🧬 Species Performance:")
        for species, perf in vs["species_performance"].items():
            print(
                f"      • {species}: {perf['n_sites']} sites, "
...
def demonstrate_edge_cases():
    ...
    single_metrics = calculator.calculate_biomass_accuracy_metrics(
        single_pred, single_obs
    )
    print(f"      RMSE: {single_metrics['rmse_biomass_kg_m2']:.4f} kg/m²")
    print(f"      MAE:  {single_metrics['mae_biomass_kg_m2']:.4f} kg/m²")
    print(
        f"      R²:   {single_metrics['r2_biomass_correlation']:.4f} (undefined for single point)"
    )
    ...
    nan_metrics = calculator.calculate_biomass_accuracy_metrics(
        pred_with_nan, obs_with_nan
    )
    ...
    # Test empty arrays
    print("\n   🔍 Empty array test:")
    empty_metrics = calculator.calculate_biomass_accuracy_metrics(
        np.array([]), np.array([])
    )
    print(
        f"      Returns default values gracefully: RMSE={empty_metrics['rmse_biomass_kg_m2']}"
    )
...
def main():
    ...
    try:
        ...
        print(
            "  • RMSE, MAE, R² calculations for biomass (kg/m²) and carbon (tC/hectare)"
        )
