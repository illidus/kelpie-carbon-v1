...
def create_sample_dataset():
    ...
    dataset = xr.Dataset(
        {
            "red": (
                ("y", "x"),
                np.random.normal(0.05, 0.02, (height, width)).clip(0, 1),
            ),
            "green": (
                ("y", "x"),
                np.random.normal(0.08, 0.03, (height, width)).clip(0, 1),
            ),
            "blue": (
                ("y", "x"),
                np.random.normal(0.06, 0.02, (height, width)).clip(0, 1),
            ),
            "nir": (
                ("y", "x"),
                np.random.normal(0.25, 0.08, (height, width)).clip(0, 1),
            ),
            "red_edge": (
                ("y", "x"),
                np.random.normal(0.15, 0.05, (height, width)).clip(0, 1),
            ),
            "swir1": (
                ("y", "x"),
                np.random.normal(0.12, 0.04, (height, width)).clip(0, 1),
            ),
        },
        coords={
    ...
    print(
        f"🌿 Kelp coverage: {np.sum(kelp_mask)} pixels ({np.sum(kelp_mask) / (height * width) * 100:.1f}%)"
    )
...
def demonstrate_enhanced_satellite_analysis():
    ...
    if "statistical_distributions" in spectral_analysis:
        ...
        for index_name, stats in spectral_analysis["statistical_distributions"].items():
            if "kelp_stats" in stats:
                ...
                print(
                    f"  {index_name.upper():12} | Kelp: {kelp_mean:6.3f} | Water: {water_mean:6.3f} | Sep: {separability:5.2f}"
                )
    ...
    try:
        temporal_analysis = analyzer.analyze_multi_temporal_changes(
            datasets, timestamps
        )
        print("✅ Temporal analysis complete")
        print(
            f"📅 Analysis period: {temporal_analysis['analysis_period']['duration_days']} days"
        )
        print(
            f"🔥 Change hotspots identified: {temporal_analysis['change_hotspots']['summary']['total_hotspots']}"
        )
    ...
    mean_uncertainty = uncertainty_analysis["summary_statistics"][
        "overall_uncertainty"
...
def demonstrate_mathematical_transparency():
    ...
    print(
        f"🌿 Total carbon: {calculation_breakdown.total_carbon:.4f} ± {calculation_breakdown.total_uncertainty:.4f} kg C/m²"
    )
    print(
        f"📊 Relative uncertainty: {(calculation_breakdown.total_uncertainty / calculation_breakdown.total_carbon) * 100:.1f}%"
    )
    print(
        f"🎯 SKEMA compatibility: {calculation_breakdown.metadata['skema_compatibility']:.1%}"
    )
...
def demonstrate_professional_reports():
    ...
    sample_data = {
        "carbon_results": type(
            "obj", (object,), {"total_carbon": 0.315, "total_uncertainty": 0.047}
        )(),
        "key_findings": [
    ...
    stakeholder_reports = report_generator.generate_stakeholder_reports(
        data=sample_data, base_config=scientific_config, output_dir=str(output_dir)
    )
...
def main():
    ...
    try:
        ...
        spectral_analysis, uncertainty_analysis = (
            demonstrate_enhanced_satellite_analysis()
        )
