...
def setup_logging(verbose: bool = False):
    ...
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(), logging.FileHandler("optimization.log")],
    )
...
def run_optimization(validation_results_path: str, output_dir: str) -> None:
    ...
    try:
        ...
        optimization_results = optimize_detection_pipeline(
            validation_results_path, output_dir
        )
        ...
        for scenario_name, config in scenarios.items():
            ...
            if "min_detection_threshold" in config:
                logger.info(
                    f"      Min detection: {config['min_detection_threshold']:.3f}"
                )
        ...
        logger.info(
            f"      NDRE: {kelp_farm_config['ndre_threshold']:.3f}, "
        ...
        ocean_config = optimizer.create_adaptive_config(
            "open_ocean", {"cloud_cover": 0.10, "turbidity": "low"}
        )
        logger.info("   🌊 Open Ocean (clear conditions):")
        logger.info(
            f"      NDRE: {ocean_config['ndre_threshold']:.3f}, "
        ...
        logger.info(
            f"      NDRE: {realtime_config['ndre_threshold']:.3f}, "
...
Examples:
  ...
  python scripts/run_threshold_optimization.py --demo-only
    ...
    parser.add_argument(
        "--validation-results", type=str, help="Path to validation results JSON file"
    )
    ...
    try:
        if args.demo_only:
            ...
            if not args.validation_results:
                logger.error(
                    "❌ --validation-results is required unless using --demo-only"
                )
                parser.print_help()
                sys.exit(1)
            ...
            if not Path(args.validation_results).exists():
                logger.error(
                    f"❌ Validation results file not found: {args.validation_results}"
                )
                sys.exit(1)
