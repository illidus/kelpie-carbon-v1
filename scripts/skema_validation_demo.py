    ...
    class SKEMAValidationDemo:
        ...
        def create_validation_sites(self):
            ...
            for site in sites:
                ...
                print(
                    f"      🔬 SKEMA: {site['skema_results']['total_kelp_area_ha']:.1f} ha, {site['skema_results']['accuracy']:.1%} accuracy"
                )
                print(
                    f"      🚀 Ours:  {site['our_results']['total_kelp_area_ha']:.1f} ha, {site['our_results']['accuracy']:.1%} accuracy"
                )
        ...
        def create_visual_demonstration(self):
            ...
            fig.suptitle(
                "SKEMA vs Our Pipeline: Satellite Processing Comparison",
                fontsize=14,
                fontweight="bold",
            )
            ...
            axes[1, 2].bar(
                x - width / 2, accuracies, width, label="Accuracy", alpha=0.8
            )
        ...
        def perform_statistical_benchmarking(self):
            ...
            for site in self.validation_sites:
                ...
                area_ratio = (
                    ...
                    / site["skema_results"]["total_kelp_area_ha"]
                )
                ...
                print(f"   📈 {site['name']}:")
                print(
                    f"      SKEMA: {skema_acc:.1%} | Ours: {our_acc:.1%} | r={correlation:.3f}"
                )
            ...
            sites_better = sum(
                1 for r in results if r["our_accuracy"] > r["skema_accuracy"]
            )
            ...
            print(
                f"      Statistically Significant: {significant_sites}/{len(results)}"
            )
        ...
        def generate_validation_report(self):
            ...
            avg_skema_acc = np.mean(
                [r["skema_accuracy"] for r in self.benchmark_results]
            )
            ...
            sites_better = sum(
                ...
                if r["our_accuracy"] > r["skema_accuracy"]
            )
            avg_equiv = np.mean(
                [f["equivalence"] for f in self.formula_comparisons.values()]
            )
