...
class TestAnalyzer:
    ...
    def analyze_test_function(
        ...
        test_info = {
            ...
            "is_parameterized": any(
                "parametrize" in str(d) for d in node.decorator_list
            ),
        }
    ...
    def group_by_function_under_test(self):
        ...
        for test in self.test_functions:
            ...
            function_name = re.sub(
                r"_(with|using|when|should|can|if)_.*", "", function_name
            )
        ...
        self.duplicate_groups["by_function"] = {
            func: tests for func, tests in function_groups.items() if len(tests) > 1
        }

        print(
            f"Found {len(self.duplicate_groups['by_function'])} function groups with potential duplicates"
        )
    ...
    def group_by_similarity(self):
        ...
        self.duplicate_groups["by_signature"] = {
            sig: tests for sig, tests in signature_groups.items() if len(tests) > 1
        }

        print(
            f"Found {len(self.duplicate_groups['by_signature'])} signature groups with potential duplicates"
        )
    ...
    def analyze_parameterization_potential(self, tests: list[dict]) -> dict:
        ...
        can_parameterize = (
            len(common_assertions) > 0 and len(common_calls) > 0 and len(tests) >= 3
        )
        ...
        return {
            ...
            "tests": tests,
            "estimated_reduction": len(tests)
            - 1,  # All tests become 1 parameterized test
        }
    ...
    def generate_consolidation_plan(self):
        ...
        for group_name, candidate in self.duplicate_groups[
            ...
            opportunity = {
                ...
                "files_affected": list(
                    set(test["file"] for test in candidate["tests"])
                ),
            }
        ...
        for sig, tests in self.duplicate_groups["by_signature"].items():
            if len(tests) > 1:
                ...
                if not any(
                    func in self.duplicate_groups["parameterization_candidates"]
    ...
    def print_summary(self):
        ...
        print(
            f"   • Parameterization candidates: {plan['analysis']['parameterization_candidates']}"
        )

        print("\n⚡ Consolidation Opportunities:")
        for opp in plan["consolidation_opportunities"][:10]:  # Show top 10
            print(
                f"   • {opp['type']}: {opp.get('function', opp.get('signature', 'N/A')[:12])} "
        ...
        if len(plan["consolidation_opportunities"]) > 10:
            print(
                f"   ... and {len(plan['consolidation_opportunities']) - 10} more opportunities"
            )

        print("\n💾 Estimated Savings:")
        print(f"   • Tests removed: {plan['estimated_savings']['tests_removed']}")
        print(
            f"   • Reduction: {plan['estimated_savings']['reduction_percentage']:.1f}%"
        )
        ...
        if plan["estimated_savings"]["reduction_percentage"] >= target_reduction:
            ...
            print(
                f"   ⚠️  Target {target_reduction}% reduction may require additional consolidation"
            )
