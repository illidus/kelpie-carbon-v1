...
from .utils import create_rgba_overlay, array_to_image, normalize_to_0_1, apply_colormap
...
def generate_confidence_overlay(
    ...
    alpha_values = (values * 255).astype(np.uint8)
...
def generate_gradient_overlay(
    ...
    for i in range(3):  # RGB channels
        rgba[:, :, i] = (
            start_color[i] * (1 - normalized) + 
            end_color[i] * normalized
        ).astype(np.uint8)
