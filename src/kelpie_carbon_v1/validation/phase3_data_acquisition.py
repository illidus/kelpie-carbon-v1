...
class Phase3DataAcquisition:
    ...
    def get_validation_sites(self, 
                           ...
                           confidence: Optional[str] = None) -> List[ValidationSite]:
        """
    ...
    def _simulate_cloud_coverage(self, site: ValidationSite, date: datetime.datetime) -> float:
        ...
        region_key = next((k for k in base_cloud.keys() if k in site.region), "California")
        ...
        if site.coordinates[0] > 0:  # Northern Hemisphere
            ...
            if 12 <= month or month <= 2:  # Summer (Dec-Feb)
                seasonal_factor = 0.8
...
def create_phase3_data_acquisition(data_directory: Optional[str] = None) -> Phase3DataAcquisition:
    """
...
def get_validation_sites(region: Optional[str] = None, 
                        species: Optional[str] = None,
                        confidence: Optional[str] = None) -> List[ValidationSite]:
    """
    Get validation sites with optional filtering.
