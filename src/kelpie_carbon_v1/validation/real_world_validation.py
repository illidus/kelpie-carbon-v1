...
class RealWorldValidator:
    ...
    async def validate_all_sites(self, start_date: str, end_date: str) -> Dict[str, ValidationResult]:
        ...
        for site_name, site in self.sites.items():
            logger.info(f"Validating site: {site.name} ({site.lat:.4f}, {site.lng:.4f})")
            ...
            try:
                ...
                if result.success:
                    logger.info(f"✅ {site.name}: {result.detection_rate:.1%} detection rate "
                              f"(expected {site.expected_detection_rate:.1%})")
                else:
                    logger.warning(f"❌ {site.name}: Validation failed - {result.error_message}")
    ...
    async def validate_site(self, site: ValidationSite, start_date: str, end_date: str) -> ValidationResult:
        ...
        try:
            ...
            if cloud_cover > self.config["max_cloud_cover"]:
                logger.warning(f"{site.name}: High cloud cover {cloud_cover}%, may affect results")
            ...
            detection_mask = create_skema_kelp_detection_mask(filtered_imagery, self.skema_config)
            ...
            success = self._evaluate_detection_success(site, detection_rate, cloud_cover)
    ...
    def _evaluate_detection_success(self, site: ValidationSite, detection_rate: float, cloud_cover: float) -> bool:
        ...
        if site.site_type == "kelp_farm":
            ...
            if not success:
                logger.warning(f"{site.name}: Detection rate {detection_rate:.1%} below "
            ...
            if not success:
                logger.warning(f"{site.name}: False positive rate {detection_rate:.1%} exceeds "
    ...
    def _log_validation_summary(self, results: Dict[str, ValidationResult]) -> None:
        ...
        successful_results = [r for r in results.values() if r.success and r.processing_time > 0]
...
async def validate_primary_sites(date_range_days: int = 30) -> Dict[str, ValidationResult]:
...
async def validate_with_controls(date_range_days: int = 30) -> Dict[str, ValidationResult]:
...
def create_real_world_validator(validation_config: Optional[Dict] = None) -> RealWorldValidator:
...
async def run_comprehensive_validation(date_range_days: int = 30) -> Dict[str, ValidationResult]:
