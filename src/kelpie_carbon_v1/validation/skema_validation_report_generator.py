...
class SKEMAValidationReportGenerator:
    ...
    def __init__(self, output_dir: str = "validation_reports"):
        ...
        print(f"🔬 SKEMA Validation Framework initialized")
    ...
    def create_validation_sites(self) -> List[ValidationSite]:
        ...
        for site in validation_sites:
            ...
            site.satellite_imagery = self.visual_demonstrator.create_synthetic_satellite_imagery(
                site.coordinates
            )
    ...
    def perform_visual_demonstrations(self) -> Dict[str, Any]:
        ...
        for site in self.validation_sites[:2]:  # Demonstrate on first 2 sites to save time
            ...
            # Save figure
            output_path = self.output_dir / f"visual_demo_{site.name.replace(' ', '_').lower()}.png"
        ...
        for site in self.validation_sites:
            ...
            benchmark = BenchmarkResults(
                ...
                performance_metrics={
                    ...
                    'area_difference_percent': abs(site.our_results['total_kelp_area_ha'] - site.skema_ground_truth['total_kelp_area_ha']) / site.skema_ground_truth['total_kelp_area_ha'] * 100,
                    'biomass_difference_percent': abs(site.our_results['biomass_tonnes'] - site.skema_ground_truth['biomass_tonnes']) / site.skema_ground_truth['biomass_tonnes'] * 100
                }
            )
    ...
    def _calculate_rmse(self, site: ValidationSite) -> float:
        ...
        area_diff = (site.our_results['total_kelp_area_ha'] - site.skema_ground_truth['total_kelp_area_ha']) / site.skema_ground_truth['total_kelp_area_ha']
        biomass_diff = (site.our_results['biomass_tonnes'] - site.skema_ground_truth['biomass_tonnes']) / site.skema_ground_truth['biomass_tonnes']
    ...
    def _calculate_bias(self, site: ValidationSite) -> float:
        ...
        area_bias = (site.our_results['total_kelp_area_ha'] - site.skema_ground_truth['total_kelp_area_ha']) / site.skema_ground_truth['total_kelp_area_ha']
        biomass_bias = (site.our_results['biomass_tonnes'] - site.skema_ground_truth['biomass_tonnes']) / site.skema_ground_truth['biomass_tonnes']
    ...
    def _calculate_significance(self, site: ValidationSite) -> float:
        ...
        area_diff = abs(site.our_results['total_kelp_area_ha'] - site.skema_ground_truth['total_kelp_area_ha']) / site.skema_ground_truth['total_kelp_area_ha']
    ...
    def _calculate_confidence_interval(self, site: ValidationSite) -> Tuple[float, float]:
        """Calculate 95% confidence interval for the comparison."""
        
        # Use confidence scores to estimate interval
        combined_confidence = (site.skema_ground_truth['confidence_score'] + site.our_results['confidence_score']) / 2
        margin = (1 - combined_confidence) * 0.2  # Higher uncertainty = wider interval
        
        area_ratio = site.our_results['total_kelp_area_ha'] / site.skema_ground_truth['total_kelp_area_ha']
...
"""
        ...
        avg_correlation = np.mean([b.correlation_coefficient for b in self.benchmark_results])
        sites_we_outperform = sum(1 for b in self.benchmark_results if b.our_accuracy > b.skema_accuracy)
...
"""
        ...
        for benchmark in self.benchmark_results:
            significance_text = "***" if benchmark.statistical_significance < 0.01 else "**" if benchmark.statistical_significance < 0.05 else "*" if benchmark.statistical_significance < 0.10 else "ns"
...
"""
    ...
    def save_validation_data(self) -> None:
        ...
        # Save to JSON
        data_path = self.output_dir / f"validation_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
