...
from .data_manager import ValidationDataManager, GroundTruthMeasurement, ValidationCampaign
...
class FieldSurveyRecord:
    ...
    mixed_species_ratio: Optional[Dict[str, float]] = None
...
class FieldDataIngestor:
    ...
    def _convert_dict_to_record(self, data: Dict[str, Any]) -> Optional[FieldSurveyRecord]:
    ...
    def _parse_species_list(self, species_str: Union[str, List[str]]) -> List[str]:
...
class SpeciesValidationAnalyzer:
    ...
    def _calculate_species_classification_metrics(
        ...
        correct_predictions = sum(1 for pred, actual in zip(predicted_species, actual_species) if pred == actual)
        ...
        for species in all_species:
            ...
            tp = sum(1 for pred, actual in zip(predicted_species, actual_species) 
                    if pred == species and actual == species)
            fp = sum(1 for pred, actual in zip(predicted_species, actual_species) 
                    if pred == species and actual != species)
            fn = sum(1 for pred, actual in zip(predicted_species, actual_species) 
                    if pred != species and actual == species)
        ...
        for actual_species_name in all_species:
            ...
            for predicted_species_name in all_species:
                count = sum(1 for pred, actual in zip(predicted_species, actual_species) 
