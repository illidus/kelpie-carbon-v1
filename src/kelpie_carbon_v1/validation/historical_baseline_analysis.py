...
class ChangeDetectionAnalyzer:
    ...
    def detect_significant_changes(
        ...
        try:
            ...
            relative_change = (absolute_change / hist_mean) * 100 if hist_mean > 0 else 0
            ...
            change_magnitude = "large" if abs(relative_change) > 25 else "medium" if abs(relative_change) > 10 else "small"
            ...
            return {
                ...
                "sample_sizes": {"historical": len(hist_values), "current": len(curr_values)}
            }
    ...
    def analyze_change_patterns(
        ...
        try:
            ...
            slope, intercept, r_value, p_value, std_err = stats.linregress(all_years, all_extents)
            ...
            decades = {}
            for year, extent in zip(all_years, all_extents):
                decade = (year // 10) * 10
            ...
            return {
                ...
                "variability_metrics": {
                    ...
                    "historical_cv": np.std(extents) / np.mean(extents) if np.mean(extents) > 0 else 0,
                    "range": max(all_extents) - min(all_extents)
                },
                "baseline_comparison": {
                    "baseline_extent": historical_dataset.baseline_extent,
                    "current_vs_baseline": current_extent - historical_dataset.baseline_extent,
                    "percent_change_from_baseline": ((current_extent - historical_dataset.baseline_extent) / historical_dataset.baseline_extent) * 100 if historical_dataset.baseline_extent > 0 else 0
                }
            }
...
class TemporalTrendAnalyzer:
        ...
        try:
            ...
            return {
                ...
                "data_quality": {
                    ...
                    "data_completeness": len([x for x in extents if x > 0]) / len(extents),
                    "average_confidence": np.mean([temporal_data[year].get("confidence", 0.8) for year in years])
                }
            }
    ...
    def _calculate_trend_metrics(self, years: List[int], extents: List[float]) -> Dict[str, Any]:
        ...
        poly_r2 = 1 - (np.sum((extents - np.polyval(poly_coeffs, years))**2) / 
    ...
    def _analyze_seasonal_patterns(self, temporal_data: Dict[int, Dict[str, float]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "periodicity_strength": np.abs(fft[dominant_freq_idx]) / np.sum(np.abs(fft)),
            "note": "Basic periodicity analysis - extend for seasonal data"
        }
    
    def _detect_cyclical_patterns(self, years: List[int], extents: List[float]) -> Dict[str, Any]:
        ...
        autocorr = [np.corrcoef(detrended[:-lag], detrended[lag:])[0,1] 
    ...
    def _generate_forecast(self, years: List[int], extents: List[float]) -> Dict[str, Any]:
        ...
        forecast_std = std_err * np.sqrt(1 + 1/len(years) + 
                                       (np.array(future_years) - np.mean(years))**2 / 
                                       np.sum((np.array(years) - np.mean(years))**2))
        
        confidence_intervals = [(pred - 1.96*std, pred + 1.96*std) 
                              for pred, std in zip(linear_forecast, forecast_std)]
...
class HistoricalBaselineAnalysis:
    ...
    def digitize_historical_data(
        ...
        quality_control_params: Optional[Dict[str, Any]] = None
    ) -> HistoricalDataset:
        """
        ...
        baseline_years = [year for year in processed_data.keys() 
                         if site.historical_period[0] <= year <= site.historical_period[0] + 10]
        baseline_extent = np.mean([processed_data[year]["extent"] for year in baseline_years])
    ...
    def _apply_quality_control(
        ...
        for year in years:
            ...
            if last_processed_year is not None:
                ...
                if change_ratio > max_extent_change or change_ratio < 1/max_extent_change:
                    logger.warning(f"Large extent change detected for year {year}: {change_ratio:.2f}x")
    ...
    def _calculate_data_quality_metrics(
        self, 
        processed_data: Dict[int, Dict[str, float]],
        site: HistoricalSite
    ) -> Dict[str, float]:
        ...
        return {
            ...
            "data_completeness": len([d for d in processed_data.values() if d["extent"] > 0]) / len(processed_data)
        }
    ...
    def perform_comprehensive_analysis(
        ...
        historical_data = {year: data["extent"] for year, data in dataset.temporal_data.items()}
        ...
        trend_results = self.trend_analyzer.analyze_temporal_trends(
            dataset,
            include_forecast=options.get("include_forecast", True)
        )
    ...
    def generate_comparison_report(
        ...
        if not all(name in self.historical_datasets for name in site_names):
            missing = [name for name in site_names if name not in self.historical_datasets]
        ...
        for site_name in site_names:
            ...
            site_stats = {
                ...
                "region": dataset.site.region,
                "species": dataset.site.species,
                "data_quality_score": np.mean(list(dataset.data_quality_metrics.values()))
            }
            
            comparative_results[site_name] = site_stats
        
        # Cross-site analysis
        all_baselines = [results["baseline_extent"] for results in comparative_results.values()]
        ...
        summary_stats = {
            ...
            "regional_diversity": len(set(results["region"] for results in comparative_results.values())),
            "species_diversity": len(set(tuple(results["species"]) for results in comparative_results.values()))
        }
    ...
    def export_results(
        ...
        export_data = {
            ...
            "export_metadata": {
                ...
                "software_version": "kelpie_carbon_v1"
            }
        }
    ...
    def _create_visualization_plots(self, dataset: HistoricalDataset, output_dir: Path) -> None:
        ...
        ax1.axhline(y=dataset.baseline_extent, color='r', linestyle='--', alpha=0.7, label='Baseline')
        ax1.set_xlabel('Year')
        ...
        ax3.axvline(x=dataset.baseline_extent, color='r', linestyle='--', alpha=0.7, label='Baseline')
        ...
        plt.tight_layout()
        plt.savefig(output_dir / f"{dataset.site.name}_historical_analysis.png", 
...
def create_uvic_historical_sites() -> Dict[str, HistoricalSite]:
    ...
    sites["broughton"] = analyzer.create_historical_site(
        ...
        data_sources=["British Admiralty Charts", "Canadian Hydrographic Service", "UVic SKEMA"],
