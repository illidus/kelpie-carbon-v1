...
from ..processing.derivative_features import DerivativeFeatures
from .environmental_testing import EnvironmentalRobustnessValidator, EnvironmentalCondition
from .real_world_validation import ValidationSite, RealWorldValidator
...
class TemporalValidator:
    ...
    async def validate_temporal_persistence(
        ...
        sampling_dates = self._generate_sampling_dates(start_date, end_date, sampling_interval_days)
        ...
        environmental_correlations = self._analyze_environmental_correlations(data_points)
        ...
        logger.info(f"Completed temporal validation with {len(data_points)} data points")
    ...
    def _simulate_environmental_conditions(
        ...
        water_temperature = temp_base + 8.0 * np.sin(2 * np.pi * (day_of_year - 60) / 365.25)
        
        # Secchi depth (water clarity)
        secchi_depth = 6.0 + 2.0 * np.sin(2 * np.pi * (day_of_year - 120) / 365.25)
        
        # Wind speed (random with seasonal bias)
        wind_speed = np.random.lognormal(2.0, 0.5) + 2.0 * np.sin(2 * np.pi * day_of_year / 365.25)
    ...
    def _analyze_seasonal_patterns(
        ...
        for season in ['spring', 'summer', 'fall', 'winter']:
            ...
            variability_coeff = season_data['detection_rate'].std() / max(avg_detection, 0.001)
    ...
    def _calculate_persistence_metrics(
        ...
        persistent_detections = sum(1 for rate in detection_rates if rate >= detection_threshold)
    ...
    def _calculate_data_quality_score(self, data_points: List[TemporalDataPoint]) -> float:
    ...
    def _perform_trend_analysis(self, data_points: List[TemporalDataPoint]) -> Dict[str, Any]:
        ...
        # Linear trend
        slope, intercept, r_value, p_value, std_err = stats.linregress(timestamps, detection_rates)
        ...
        if len(detection_rates) > 5:
            ...
            for i in range(2, len(detection_rates) - 2):
                ...
                after_avg = np.mean(detection_rates[i+1:min(len(detection_rates), i+3)])
        ...
        return {
            ...
            'variability_analysis': {
                ...
                'trend_adjusted_variance': float(np.var(detection_rates) - slope**2 * np.var(timestamps)),
                'periodic_component': self._estimate_periodic_component(data_points)
            }
        }
    
    def _estimate_periodic_component(self, data_points: List[TemporalDataPoint]) -> float:
    ...
    def _assess_temporal_quality(
        ...
        quality_checks = {
            'meets_minimum_detection': mean_detection >= thresholds["minimum_detection_rate"],
            'meets_consistency_threshold': detection_consistency >= thresholds["consistency_threshold"],
    ...
    def _calculate_detection_consistency(self, data_points: List[TemporalDataPoint]) -> float:
        """Calculate detection consistency across temporal samples."""
        if len(data_points) < 2:
            return 0.0
        
        detection_rates = [dp.detection_rate for dp in data_points]
        coefficient_of_variation = np.std(detection_rates) / max(np.mean(detection_rates), 0.001)
    ...
    def _identify_temporal_gaps(self, data_points: List[TemporalDataPoint]) -> List[Dict[str, str]]:
        ...
        for i in range(1, len(sorted_points)):
            gap_days = (sorted_points[i].timestamp - sorted_points[i-1].timestamp).days
            
            if gap_days > 30:  # Gap longer than 30 days
                gaps.append({
                    'start_date': sorted_points[i-1].timestamp.strftime('%Y-%m-%d'),
    ...
    def _generate_quality_recommendations(self, quality_checks: Dict[str, bool]) -> List[str]:
        """Generate recommendations based on quality assessment."""
        recommendations = []
        
        if not quality_checks.get('meets_minimum_detection', True):
            recommendations.append("Increase detection sensitivity or verify site has sufficient kelp presence")
        
        if not quality_checks.get('meets_consistency_threshold', True):
            recommendations.append("Investigate sources of temporal inconsistency in detection")
        
        if not quality_checks.get('sufficient_data_points', True):
            recommendations.append("Collect additional temporal samples for robust analysis")
        ...
        if not recommendations:
            recommendations.append("Temporal validation quality is acceptable for production use")
    ...
    def _generate_temporal_recommendations(
        ...
        if persistence_metrics.get('persistence_rate', 0) < 0.6:
            recommendations.append("Low persistence rate - consider site-specific calibration")
        
        if persistence_metrics.get('coefficient_of_variation', 1) > 0.5:
            recommendations.append("High temporal variability - implement adaptive thresholding")
        
        # Seasonal recommendations
        if seasonal_patterns:
            summer_pattern = seasonal_patterns.get('summer')
            if summer_pattern and summer_pattern.average_detection_rate < 0.15:
                recommendations.append("Low summer detection rates - verify optimal processing parameters")
        ...
        if trend.get('trend_strength') == 'strong' and trend.get('p_value', 1) < 0.05:
            if trend.get('slope', 0) < -0.01:
                recommendations.append("Significant declining trend detected - investigate environmental changes")
            elif trend.get('slope', 0) > 0.01:
                recommendations.append("Significant increasing trend detected - validate against ground truth")
        
        # Quality recommendations
        data_quality = persistence_metrics.get('data_quality_score', 0)
        if data_quality < 0.7:
            recommendations.append("Improve data quality through enhanced preprocessing")
        
        if not recommendations:
            recommendations.append("Temporal validation shows good stability for operational deployment")
    ...
    async def run_broughton_archipelago_validation(
        ...
        logger.info(f"Starting Broughton Archipelago {validation_years}-year temporal validation")
        
        result = await self.validate_temporal_persistence(
            site=site,
            start_date=start_date,
            end_date=end_date,
            sampling_interval_days=config["temporal_parameters"]["sampling_frequency_days"]
        )
    ...
    def generate_comprehensive_temporal_report(
        ...
        overall_assessment = self._determine_overall_assessment(summary_metrics, seasonal_summary)
        
        return {
            'executive_summary': {
                'total_sites_validated': len(results),
                'validation_period_days': sum(
                    (r.validation_period[1] - r.validation_period[0]).days for r in results
                ) // len(results),
                'overall_assessment': overall_assessment,
                'mean_persistence_rate': summary_metrics.get('mean_persistence_rate', 0.0),
                ...
                'site_specific_results': [
                    {
                        ...
                        'persistence_rate': r.persistence_metrics.get('persistence_rate', 0.0),
            ...
            'recommendations': {
                'high_priority': self._extract_high_priority_recommendations(all_recommendations),
                'operational': self._extract_operational_recommendations(all_recommendations),
            ...
            'validation_quality': {
                ...
                'overall_quality_distribution': self._calculate_quality_distribution(results)
            },
    ...
    def _calculate_summary_metrics(
        self, 
        all_persistence_metrics: List[Dict[str, float]]
    ) -> Dict[str, float]:
        ...
        for metric_name in all_persistence_metrics[0].keys():
            values = [metrics.get(metric_name, 0.0) for metrics in all_persistence_metrics]
    ...
    def _summarize_seasonal_patterns(
        self, 
        all_seasonal_patterns: Dict[str, List[SeasonalPattern]]
        ...
        for season, patterns in all_seasonal_patterns.items():
            if patterns:
                summary[season] = {
                    'mean_detection_rate': float(np.mean([p.average_detection_rate for p in patterns])),
                    'mean_variability': float(np.mean([p.variability_coefficient for p in patterns])),
                    'sites_count': len(patterns),
                    'significant_trends': sum(1 for p in patterns if p.statistical_significance < 0.05)
                }
    ...
    def _extract_high_priority_recommendations(self, all_recommendations: List[str]) -> List[str]:
        """Extract high-priority recommendations."""
        high_priority_keywords = ['declining trend', 'insufficient data', 'low detection', 'calibration']
    ...
    def _extract_operational_recommendations(self, all_recommendations: List[str]) -> List[str]:
    ...
    def _extract_research_recommendations(self, all_recommendations: List[str]) -> List[str]:
        """Extract research-oriented recommendations."""
        research_keywords = ['investigate', 'environmental', 'ground truth', 'validation']
    ...
    def _calculate_quality_distribution(self, results: List[TemporalValidationResult]) -> Dict[str, int]:
...
async def run_broughton_temporal_validation(validation_years: int = 3) -> TemporalValidationResult:
