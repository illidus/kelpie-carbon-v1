...
class RealDataAcquisition:
    ...
    def __init__(self, data_directory: Optional[str] = None):
        ...
        self.data_directory = Path(data_directory) if data_directory else Path("data/validation")
        ...
        logger.info(f"Initialized RealDataAcquisition with data directory: {self.data_directory}")
    ...
    def get_validation_sites(self, region: Optional[str] = None, species: Optional[str] = None) -> List[ValidationSite]:
        """
        ...
        logger.info(f"Found {len(sites)} sites matching filters (region={region}, species={species})")
        return sites
    ...
    def create_synthetic_ground_truth(
        ...
        site = self.validation_sites[site_id]
        logger.info(f"Creating synthetic ground truth for {len(scenes)} scenes at {site.name}")
        ...
        for scene in scenes:
            ...
            if start_month <= end_month:  # Normal season
                if start_month <= month <= end_month:
                    ...
                    seasonal_factor = 1.0 - abs(month - mid_season) / ((end_month - start_month) / 2) * 0.5
                else:
                ...
                if month >= start_month or month <= end_month:
                    ...
                    seasonal_factor = 1.0 - abs(season_pos - mid_season) / (season_length / 2) * 0.5
                else:
                    seasonal_factor = 0.1
    ...
    def create_validation_dataset(
        ...
        if use_synthetic:
            ...
            logger.warning("Real data acquisition not yet implemented, falling back to synthetic")
            scenes = self.create_synthetic_satellite_data(site_id, num_scenes)
            ground_truth = self.create_synthetic_ground_truth(site_id, scenes)
        ...
        logger.info(f"Created validation dataset with {len(scenes)} scenes and {len(ground_truth)} ground truth records")
        return dataset
    ...
    def _calculate_dataset_quality_metrics(
        ...
        excellent_scenes = sum(1 for scene in scenes if scene.data_quality == "excellent")
        good_or_better = sum(1 for scene in scenes if scene.data_quality in ["excellent", "good"])
        
        # Ground truth quality metrics
        avg_confidence = np.mean([gt.confidence for gt in ground_truth])
        coverage_range = max([gt.kelp_coverage_percent for gt in ground_truth]) - min([gt.kelp_coverage_percent for gt in ground_truth])
        ...
        overall_quality = np.mean([cloud_score, quality_score, confidence_score, temporal_score, coverage_score])
        
        return {
            "overall_quality": round(float(overall_quality), 6),
            "average_cloud_coverage": round(float(avg_cloud_coverage), 6),
            "excellent_scenes_percent": round(float(excellent_scenes / len(scenes) * 100), 6),
            "good_or_better_percent": round(float(good_or_better / len(scenes) * 100), 6),
    ...
    def save_validation_dataset(self, dataset: ValidationDataset, filename: Optional[str] = None) -> str:
        """
    ...
    def load_validation_dataset(self, filepath: str) -> ValidationDataset:
        ...
        for gt_data in dataset_dict["ground_truth"]:
            gt_data["collection_date"] = datetime.datetime.fromisoformat(gt_data["collection_date"])
    ...
    def create_benchmark_suite(self, include_sites: Optional[List[str]] = None) -> Dict[str, ValidationDataset]:
        """
        ...
        for site_id in include_sites:
            ...
            try:
                dataset = self.create_validation_dataset(site_id, num_scenes=8, use_synthetic=True)
...
def create_real_data_acquisition(data_directory: Optional[str] = None) -> RealDataAcquisition:
    """
...
def get_validation_sites(region: Optional[str] = None, species: Optional[str] = None) -> List[ValidationSite]:
...
def create_benchmark_dataset(site_id: str, num_scenes: int = 5) -> ValidationDataset:
    ...
    acquisition = create_real_data_acquisition()
    return acquisition.create_validation_dataset(site_id, num_scenes, use_synthetic=True) 