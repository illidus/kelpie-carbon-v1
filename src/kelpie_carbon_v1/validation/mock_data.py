...
from .data_manager import ValidationCampaign, GroundTruthMeasurement, ValidationDataManager
...
class MockValidationGenerator:
    ...
    def create_bc_validation_dataset(self, site_key: str = "saanich_inlet") -> str:
        ...
        for i in range(50):  # 50 measurement points
            ...
            lng = site["lng"] + random.uniform(-0.01, 0.01)
            
            kelp_present = random.random() < site["kelp_coverage"]
            ...
            measurement = GroundTruthMeasurement(
                ...
                kelp_species=site["species"] if kelp_present else None,
                kelp_density="moderate" if kelp_present else "none",
                canopy_type="surface" if depth < 10 else "submerged",
                timestamp=campaign.satellite_overpass_time + timedelta(minutes=random.randint(-60, 60)),
