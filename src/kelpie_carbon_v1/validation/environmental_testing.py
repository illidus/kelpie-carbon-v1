...
class EnvironmentalRobustnessValidator:
    ...
    def apply_water_clarity_correction(self, dataset: xr.Dataset,
                                     condition: EnvironmentalCondition) -> xr.Dataset:
        ...
        if secchi_depth < 4.0:  # Turbid conditions
            ...
            if waf_intensity > 1.0:
                ...
                corrected_dataset = self._apply_enhanced_waf(corrected_dataset, waf_intensity)
        
        return corrected_dataset
    
    def apply_seasonal_correction(self, detection_mask: np.ndarray,
                                condition: EnvironmentalCondition) -> np.ndarray:
    ...
    async def test_environmental_condition(self, condition: EnvironmentalCondition,
                                         lat: float, lng: float,
                                         start_date: str, end_date: str) -> EnvironmentalTestResult:
        ...
        try:
            ...
            if dataset is None or len(dataset.data_vars) == 0:
                ...
                return self._create_failed_result(condition, "No satellite data available")
            ...
            logger.info(f"Completed testing {condition.name}: {'PASS' if success else 'FAIL'}")
            return result
    ...
    def _apply_enhanced_waf(self, dataset: xr.Dataset, intensity: float) -> xr.Dataset:
        ...
        enhanced_config = {
            'sunglint_threshold': 0.15 / intensity,  # Lower threshold for higher intensity
            'kernel_size': max(3, int(5 * intensity)),  # Larger kernel for higher intensity
    ...
    def _evaluate_condition_success(self, detection_rate: float,
                                   condition: EnvironmentalCondition,
    ...
    def _create_failed_result(self, condition: EnvironmentalCondition, 
                            error_message: str) -> EnvironmentalTestResult:
    ...
    async def run_comprehensive_environmental_testing(self, 
                                                     lat: float, lng: float,
                                                     base_date: str,
        ...
        start_date = (base_datetime - timedelta(days=date_range_days)).strftime("%Y-%m-%d")
        ...
        logger.info(f"Environmental testing complete: {report['summary']['success_rate']*100:.1f}% success rate")
        return report
    
    # Convenience method aliases for test compatibility
    async def run_comprehensive_testing(self, lat: float, lng: float, base_date: str) -> Dict[str, Any]:
    ...
    def _generate_report(self, results: List[EnvironmentalTestResult]) -> Dict[str, Any]:
        """Convenience method alias for test compatibility."""
        return self._generate_environmental_report(results)
    
    def _generate_environmental_report(self, results: List[EnvironmentalTestResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "condition_categories": {
                "tidal_effects": {
                    "tested": len([r for r in results if "tidal" in r.condition.name or "tide" in r.condition.name]),
                    "successful": len([r for r in successful_tests if "tidal" in r.condition.name or "tide" in r.condition.name]),
                },
                "water_clarity": {
                    "tested": len([r for r in results if "turbid" in r.condition.name or "clear" in r.condition.name]),
                    "successful": len([r for r in successful_tests if "turbid" in r.condition.name or "clear" in r.condition.name]),
                },
                "seasonal_variation": {
                    "tested": len([r for r in results if "season" in r.condition.name]),
                    "successful": len([r for r in successful_tests if "season" in r.condition.name]),
                },
            },
            "detailed_results": [
                {
    ...
    def _generate_recommendations(self, results: List[EnvironmentalTestResult]) -> List[str]:
        ...
        clarity_results = [r for r in results if "turbid" in r.condition.name or "clear" in r.condition.name]
        turbid_success = any(r.success for r in clarity_results if "turbid" in r.condition.name)
        ...
        if seasonal_results:
            off_season_success = any(r.success for r in seasonal_results if "off_season" in r.condition.name)
...
async def validate_tidal_effects(lat: float, lng: float, base_date: str) -> Dict[str, Any]:
...
async def validate_water_clarity_effects(lat: float, lng: float, base_date: str) -> Dict[str, Any]:
    """Test water clarity variations on kelp detection."""
    validator = EnvironmentalRobustnessValidator()
    
    # Get only water clarity conditions
    all_conditions = validator.get_environmental_conditions()
    clarity_conditions = [c for c in all_conditions if "turbid" in c.name or "clear" in c.name]
...
async def validate_seasonal_variations(lat: float, lng: float, 
                                 peak_season_date: str, off_season_date: str) -> Dict[str, Any]:
...
async def run_comprehensive_environmental_testing(lat: float, lng: float, 
                                                base_date: str, 
                                                date_range_days: int = 30) -> Dict[str, Any]:
    """Run comprehensive environmental robustness testing.
