...
class ClassicalMLEnhancer:
    ...
    def __init__(self, 
        ...
        self.feature_config = {
            ...
            "morphological_features": ["area", "perimeter", "compactness", "eccentricity"],
            "statistical_features": ["mean", "std", "skewness", "kurtosis"]
        }
        
        print("🤖 Classical ML Enhancer initialized (zero-cost mode)")
    
    def enhance_kelp_detection(self,
                             rgb_image: np.ndarray,
                             nir_band: Optional[np.ndarray] = None,
                             red_edge_band: Optional[np.ndarray] = None) -> Tuple[np.ndarray, dict]:
        """
        ...
        metadata = self._calculate_enhancement_metrics(baseline_mask, enhanced_mask, features)
        
        print(f"✅ ML enhancement complete")
    ...
    def _extract_comprehensive_features(self,
                                      ...
                                      red_edge_band: Optional[np.ndarray],
                                      baseline_mask: np.ndarray) -> Dict[str, np.ndarray]:
    ...
    def _extract_morphological_features(self, mask: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        morph_features["distance_transform"] = distance_transform / distance_transform.max() if distance_transform.max() > 0 else distance_transform
    ...
    def _extract_statistical_features(self, 
                                    rgb_image: np.ndarray,
                                    nir_band: Optional[np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        if nir_band is not None:
            ...
            nir_local_variance = cv2.filter2D((nir_band - nir_local_mean)**2, -1, kernel)
    ...
    def _apply_ml_enhancement(self, 
                            ...
                            baseline_mask: np.ndarray) -> np.ndarray:
        ...
        if self.use_spectral_clustering:
            enhanced_mask = self._apply_spectral_clustering(feature_matrix, enhanced_mask)
    ...
    def _prepare_feature_matrix(self, features: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        for feature_name, feature_array in features.items():
            if feature_array.ndim == 2:
                ...
                print(f"⚠️  Skipping feature {feature_name} with shape {feature_array.shape}")
    ...
    def _unsupervised_enhancement(self, 
    ...
    def _apply_anomaly_detection(self, 
                               feature_matrix: np.ndarray,
                               current_mask: np.ndarray) -> np.ndarray:
        """Apply anomaly detection for noise reduction."""
        try:
            ...
            kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
            enhanced_mask = cv2.morphologyEx(enhanced_mask.astype(np.uint8), cv2.MORPH_OPEN, kernel)
    ...
    def _calculate_enhancement_metrics(self, 
        ...
        if baseline_pixels > 0:
            improvement_percentage = ((enhanced_pixels - baseline_pixels) / baseline_pixels) * 100
        else:
    ...
    def batch_enhance_directory(self,
                              input_dir: str,
                              output_dir: str,
                              pattern: str = "*.tif") -> Dict[str, Any]:
        ...
        for image_file in input_path.glob(pattern):
            try:
                ...
                print(f"✅ Enhanced {image_file.name}: {metadata['improvement_percentage']:+.1f}% improvement")
        ...
        if results["enhanced_files"]:
            results["average_improvement"] = results["total_improvement"] / len(results["enhanced_files"])
...
def setup_classical_ml_environment():
    ...
    print("   enhanced_mask, metadata = enhancer.enhance_kelp_detection(rgb, nir, red_edge)")
