...
def _find_available_port(start_port: int, max_attempts: int = Network.MAX_PORT_ATTEMPTS) -> int:
...
def serve(
    ...
    env: Optional[str] = typer.Option(None, "--env", "-e", help="Environment (development/production)"),
    auto_port: bool = typer.Option(False, "--auto-port", help="Automatically find available port if specified port is busy"),
):
    ...
    if auto_port:
        try:
            ...
            if target_port != (port or settings.port):
                logger.info(f"Port {port or settings.port} was busy, using port {target_port} instead")
    ...
    logger.info(f"Starting Kelpie Carbon v1 server on {server_config['host']}:{server_config['port']}")
    ...
    try:
        ...
        if "Address already in use" in str(e) or "10048" in str(e):
            logger.error(f"Port {target_port} is already in use. Try using --auto-port flag or specify a different port with --port")
...
def analyze(
    ...
    setup_logging()
    logger.info(f"Starting analysis for location ({lat}, {lng}) from {start_date} to {end_date}")
    
    try:
        from .core import fetch_sentinel_tiles, calculate_indices_from_dataset, apply_mask, predict_biomass
        ...
        carbon = biomass * KelpAnalysis.CARBON_CONTENT_FACTOR / KelpAnalysis.HECTARE_TO_M2  # Convert to kg C/m²
...
@app.command()
def config(
    env: Optional[str] = typer.Option("development", "--env", "-e", help="Environment to show config for"),
):
...
@app.command()
def test(
    pattern: Optional[str] = typer.Option("test_*.py", "--pattern", "-p", help="Test file pattern"),
