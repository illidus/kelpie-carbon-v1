...
from kelpie_carbon_v1.core.mask import (
    ...
    calculate_ndre,
    create_water_mask,
    _apply_ndre_detection
)
# # from kelpie_carbon_v1.core.spectral import apply_spectral_enhancement  # Module doesn't exist  # Module doesn't exist
...
class SubmergedKelpConfig:
    ...
    species_depth_factors: Dict[str, float] = field(default_factory=lambda: {
...
class SubmergedKelpDetector:
    ...
    def __init__(self, config: Optional[SubmergedKelpConfig] = None):
        """Initialize submerged kelp detector with configuration."""
        self.config = config or SubmergedKelpConfig()
        logger.info("Initialized SubmergedKelpDetector with depth-sensitive capabilities")
    ...
    def detect_submerged_kelp(
        ...
        try:
            ...
            if include_depth_analysis:
                depth_estimate, depth_confidence, water_column_props = self._estimate_kelp_depths(
            ...
            logger.info(f"Submerged kelp detection completed. "
    ...
    def _calculate_depth_sensitive_indices(self, dataset: xr.Dataset) -> Dict[str, np.ndarray]:
        """
        ...
        if self.config.primary_red_edge_band in dataset:
            red_edge_2 = dataset[self.config.primary_red_edge_band].values.astype(np.float32)
        else:
            red_edge_2 = dataset[self.config.fallback_red_edge_band].values.astype(np.float32)
    ...
    def _apply_depth_stratified_detection(
        ...
        shallow_submerged = (
            (indices['ndre_enhanced'] > shallow_threshold) & 
        ...
        logger.debug(f"Depth-stratified detection complete. "
    ...
    def _estimate_kelp_depths(
        ...
        if np.any(submerged_mask):
            ...
            submerged_warei = indices.get('warei', indices['ndre_enhanced'])[submerged_mask]
            ...
            surface_reflectance = np.mean(indices['ndre_enhanced'][surface_mask]) if np.any(surface_mask) else 0.05
            ...
            with np.errstate(divide="ignore", invalid="ignore"):
                ...
                apparent_depths = np.clip(apparent_depths, 0.3, wcm.depth_max_detectable)
        ...
        logger.debug(f"Depth estimation complete. "
    ...
    def _model_water_column_properties(
        ...
        attenuation_factor = 1.0 + 2.0 * (1.0 - water_clarity)  # Higher with lower clarity
    ...
    def _apply_quality_control(
        ...
        surface_filtered = self._remove_small_patches(surface_mask, self.config.min_patch_size)
        submerged_filtered = self._remove_small_patches(submerged_mask, self.config.min_patch_size)
        ...
        logger.debug(f"Quality control complete. "
    ...
    def _generate_detection_metadata(
        ...
        metadata = {
            ...
            "total_kelp_coverage_percent": float(total_kelp_pixels / total_pixels * 100),
            ...
            "surface_to_submerged_ratio": float(surface_pixels / max(submerged_pixels, 1)),
            ...
            "config_used": {
                ...
                "species_depth_factor": self.config.species_depth_factors.get(species, 1.0)
            }
        }
...
def create_submerged_kelp_detector(config: Optional[SubmergedKelpConfig] = None) -> SubmergedKelpDetector:
    """
...
def detect_submerged_kelp(
    dataset: xr.Dataset,
    species: str = "Mixed",
    config: Optional[SubmergedKelpConfig] = None,
    include_depth_analysis: bool = True
) -> DepthDetectionResult:
    """
...
def analyze_depth_distribution(result: DepthDetectionResult) -> Dict[str, float]:
    ...
    analysis = {
        ...
        "shallow_fraction": float(np.sum((depths > 0.3) & (depths <= 0.7)) / len(depths)),
