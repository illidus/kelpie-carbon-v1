    ...
    if arr.ndim == 1 or axis is not None:
        ...
        if axis is None:
            ...
            if method == "linear":
                ...
                if len(valid_values) >= 4:
                    f = interpolate.interp1d(valid_indices, valid_values, kind='cubic',
                                           bounds_error=False, fill_value='extrapolate')
                else:
                ...
                f = interpolate.interp1d(valid_indices, valid_values, kind='nearest',
                                       bounds_error=False, fill_value='extrapolate')
            else:
            ...
            result = np.apply_along_axis(
                lambda x: interpolate_missing_values(x, method=method), 
                axis, arr
            )
        ...
        if len(valid_values) < 3:
            ...
            interpolated = griddata(
                ...
                method=method if method != "cubic" else "linear"  # griddata doesn't support cubic
            )
