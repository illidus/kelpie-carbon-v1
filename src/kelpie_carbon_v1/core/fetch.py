...
from ..constants import SatelliteData, KelpAnalysis
...
def fetch_sentinel_tiles(
    ...
    try:
        ...
        search = catalog.search(
            ...
            query={"eo:cloud_cover": {"lt": SatelliteData.MAX_CLOUD_COVER}}  # Less than threshold cloud cover
        )
        ...
        if not items:
            ...
            logger.warning(f"No Sentinel-2 data found for {lat}, {lng} between {start_date} and {end_date}")
        ...
        logger.info(f"Cloud coverage: {best_item.properties.get('eo:cloud_cover', 'Unknown')}%")
        ...
        for band_name, asset_key in bands_to_fetch.items():
            if asset_key in signed_item.assets:
                ...
                with rxr.open_rasterio(asset_url) as da:
                    ...
                    clipped = clipped.astype(np.float32) / SatelliteData.SENTINEL_SCALE_FACTOR
        ...
        if cloud_band in signed_item.assets:
            ...
            with rxr.open_rasterio(asset_url) as da:
                ...
                cloud_mask = ((clipped == 8) | (clipped == 9) | (clipped == 10)).astype(np.int32)
        ...
        for band_name, data_array in data_arrays.items():
            ...
            data_array = data_array.interp(x=coords["x"], y=coords["y"], method="nearest")
        ...
        return {
            ...
            "bands": list(bands_to_fetch.keys()) + (["cloud_mask"] if cloud_mask is not None else []),
...
def _create_mock_sentinel_data(
    ...
    # Water areas
    red[water_mask] = np.random.normal(0.02, 0.01, np.sum(water_mask)).clip(0, 0.1)
    red_edge[water_mask] = np.random.normal(0.03, 0.01, np.sum(water_mask)).clip(0, 0.1)
    red_edge_2[water_mask] = np.random.normal(0.025, 0.008, np.sum(water_mask)).clip(0, 0.08)  # Slightly lower
    red_edge_3[water_mask] = np.random.normal(0.015, 0.005, np.sum(water_mask)).clip(0, 0.06)  # Approaching NIR
    nir[water_mask] = np.random.normal(0.01, 0.005, np.sum(water_mask)).clip(0, 0.05)
    swir1[water_mask] = np.random.normal(0.005, 0.002, np.sum(water_mask)).clip(0, 0.02)
    ...
    red[veg_mask] = np.random.normal(0.04, 0.02, np.sum(veg_mask)).clip(0, 0.15)
    red_edge[veg_mask] = np.random.normal(0.15, 0.05, np.sum(veg_mask)).clip(0, 0.4)
    red_edge_2[veg_mask] = np.random.normal(0.20, 0.06, np.sum(veg_mask)).clip(0.05, 0.45)  # Peak red-edge
    red_edge_3[veg_mask] = np.random.normal(0.30, 0.08, np.sum(veg_mask)).clip(0.15, 0.65)  # Transitioning to NIR
    nir[veg_mask] = np.random.normal(0.4, 0.1, np.sum(veg_mask)).clip(0.2, 0.8)
    swir1[veg_mask] = np.random.normal(0.25, 0.05, np.sum(veg_mask)).clip(0.1, 0.5)
    
    # Non-vegetated land
    bare_mask = land_mask & ~vegetation_prob
    red[bare_mask] = np.random.normal(0.15, 0.05, np.sum(bare_mask)).clip(0.05, 0.4)
    red_edge[bare_mask] = np.random.normal(0.18, 0.05, np.sum(bare_mask)).clip(0.1, 0.4)
    red_edge_2[bare_mask] = np.random.normal(0.20, 0.05, np.sum(bare_mask)).clip(0.12, 0.42)  # Similar to red_edge
    red_edge_3[bare_mask] = np.random.normal(0.22, 0.06, np.sum(bare_mask)).clip(0.14, 0.45)  # Slightly higher
    nir[bare_mask] = np.random.normal(0.25, 0.08, np.sum(bare_mask)).clip(0.1, 0.5)
    swir1[bare_mask] = np.random.normal(0.3, 0.08, np.sum(bare_mask)).clip(0.15, 0.6)
    ...
    # Random kelp patches
    kelp_probability = np.random.random((height, width)) < 0.1  # 10% chance in coastal waters
    kelp_mask = coastline_buffer & kelp_probability
    
    # Kelp spectral signature - Based on SKEMA research findings
    red[kelp_mask] = np.random.normal(0.02, 0.005, np.sum(kelp_mask)).clip(0, 0.05)
    red_edge[kelp_mask] = np.random.normal(0.08, 0.02, np.sum(kelp_mask)).clip(0.04, 0.15)
    red_edge_2[kelp_mask] = np.random.normal(0.12, 0.025, np.sum(kelp_mask)).clip(0.06, 0.18)  # Higher for submerged detection
    red_edge_3[kelp_mask] = np.random.normal(0.09, 0.02, np.sum(kelp_mask)).clip(0.05, 0.14)  # Lower than peak red-edge
    nir[kelp_mask] = np.random.normal(0.06, 0.02, np.sum(kelp_mask)).clip(0.02, 0.12)
    swir1[kelp_mask] = np.random.normal(0.01, 0.005, np.sum(kelp_mask)).clip(0, 0.03)
    ...
    return {
        ...
        "bands": ["red", "red_edge", "red_edge_2", "red_edge_3", "nir", "swir1", "cloud_mask"],
    }
