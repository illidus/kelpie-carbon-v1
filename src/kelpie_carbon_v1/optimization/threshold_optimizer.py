...
class ThresholdOptimizer:
    ...
    def load_validation_results(self, results_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Results contain {results['total_sites']} sites with "
    ...
    def analyze_detection_rates(self, results: Dict[str, Any]) -> Dict[str, float]:
        ...
        analysis['over_detection_ratio'] = analysis['mean_detection_rate'] / analysis['mean_expected_rate']
        
        # Calculate accuracy score (1.0 = perfect, 0.0 = worst)
        # Penalize both over-detection and under-detection
        detection_errors = [abs(actual - expected) for actual, expected in zip(detection_rates, expected_rates)]
    ...
    def calculate_optimal_thresholds(self, results: Dict[str, Any]) -> Dict[str, float]:
        ...
        optimal_thresholds = {
            ...
            'min_detection_threshold': results['configuration'].get('min_detection_threshold', 0.01),
        ...
        if analysis['over_detection_ratio'] > 2.0:
            ...
            optimal_thresholds['ndre_threshold'] = max(0.05, current_config.get('ndre_threshold', 0.0) + 0.1)
            optimal_thresholds['min_detection_threshold'] = min(0.2, 
                results['configuration'].get('min_detection_threshold', 0.01) * threshold_multiplier)
            optimal_thresholds['kelp_fai_threshold'] = min(0.1, 0.01 * threshold_multiplier)
            
        elif analysis['over_detection_ratio'] > 1.5:
            # Moderate threshold increase
            optimal_thresholds['ndre_threshold'] = max(0.02, current_config.get('ndre_threshold', 0.0) + 0.05)
            optimal_thresholds['min_detection_threshold'] = min(0.1,
                results['configuration'].get('min_detection_threshold', 0.01) * 3.0)
            ...
            optimal_thresholds['ndre_threshold'] = max(0.0, current_config.get('ndre_threshold', 0.0) - 0.02)
            optimal_thresholds['min_detection_threshold'] = max(0.005,
                results['configuration'].get('min_detection_threshold', 0.01) * 0.5)
            optimal_thresholds['kelp_fai_threshold'] = max(0.005, 0.01 * 0.5)
        
        logger.info(f"Calculated optimal thresholds:")
        for key, value in optimal_thresholds.items():
            logger.info(f"  {key}: {value:.3f}")
        
        return optimal_thresholds

    def create_adaptive_config(self, site_type: str, environmental_conditions: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"  Environmental conditions: cloud_cover={cloud_cover:.1%}, turbidity={turbidity}")
        
        return config

    def optimize_for_real_time(self, target_processing_time: float = 15.0) -> Dict[str, Any]:
        ...
        logger.info(f"Created real-time optimized config (target: {target_processing_time}s)")
        return fast_config

    def save_optimization_results(self, output_path: str, optimization_type: str = "threshold_tuning"):
        ...
        self.optimization_results['total_validations_analyzed'] = len(self.validation_history)
    ...
    def run_comprehensive_optimization(self, validation_results_path: str, output_dir: str = "results/optimization/") -> Dict[str, Any]:
        ...
        scenarios = {
            ...
            'kelp_farm_tuned': self.create_adaptive_config('kelp_farm', {'cloud_cover': 0.15, 'turbidity': 'medium'}),
            'open_ocean_tuned': self.create_adaptive_config('open_ocean', {'cloud_cover': 0.20, 'turbidity': 'low'}),
            'coastal_tuned': self.create_adaptive_config('coastal', {'cloud_cover': 0.25, 'turbidity': 'high'}),
        ...
        self.save_optimization_results(f"{output_dir}/optimization_results_{timestamp}.json")
    ...
    def _generate_recommendations(self, analysis: Dict[str, float]) -> List[str]:
        ...
        if analysis['over_detection_ratio'] > 3.0:
            recommendations.append("CRITICAL: Detection thresholds are too low, causing severe over-detection")
            recommendations.append("Increase NDRE threshold to at least 0.1 and FAI threshold to 0.05")
            
        elif analysis['over_detection_ratio'] > 2.0:
            recommendations.append("HIGH: Significant over-detection detected")
            recommendations.append("Increase detection thresholds by 2-3x current values")
            ...
            recommendations.append("Under-detection detected - consider lowering thresholds")
            
        if analysis['accuracy_score'] < 0.3:
            recommendations.append("Poor accuracy score - comprehensive threshold review needed")
        ...
        recommendations.append("Consider adaptive thresholding based on environmental conditions")
        recommendations.append("Implement real-time optimization for production deployments")
...
def optimize_detection_pipeline(validation_results_path: str, output_dir: str = "results/optimization/") -> Dict[str, Any]:
...
def get_optimized_config_for_site(site_type: str, environmental_conditions: Dict[str, Any]) -> Dict[str, Any]:
