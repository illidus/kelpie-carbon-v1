...
class PathsConfig:
    ...
    static_files: str = "src/kelpie_carbon_v1/web/static"
    templates: str = "src/kelpie_carbon_v1/web/templates"
    logs: str = "logs"
    cache: str = "cache"
    temp: str = "tmp"
...
class ConfigLoader:
    ...
    def load_config_file(self, filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"Failed to load config file {config_path}: {e}")
            return {}
    ...
    def create_config(self, environment: Optional[str] = None) -> Config:
    ...
    def _dict_to_config(self, config_dict: Dict[str, Any]) -> Config:
        ...
        config.satellite = SatelliteConfig(
            data_source=satellite_config.get("data_source", config.satellite.data_source),
        ...
        config.ml_models = MLModelsConfig(
            ...
            kelp_detection_confidence_threshold=kelp_detection.get(
                "confidence_threshold", config.ml_models.kelp_detection_confidence_threshold
            ),
        ...
        config.security = SecurityConfig(
            trusted_hosts=security_config.get("trusted_hosts", config.security.trusted_hosts),
