...
def log_performance(func_name: str, duration: float, **kwargs) -> None:
    ...
    if duration > 5.0:
        logger.warning(f"Slow operation: {func_name} took {duration:.2f}s", extra=extra_data)
    else:
...
def log_api_request(method: str, path: str, status_code: int, duration: float, **kwargs) -> None:
    ...
    if status_code >= 500:
        logger.error(f"{method} {path} -> {status_code} ({duration:.3f}s)", extra=extra_data)
    elif status_code >= 400:
        logger.warning(f"{method} {path} -> {status_code} ({duration:.3f}s)", extra=extra_data)
    else:
        logger.info(f"{method} {path} -> {status_code} ({duration:.3f}s)", extra=extra_data)


def log_satellite_data_fetch(location: Dict[str, float], date_range: Dict[str, str], 
    ...
    if success:
        logger.info(f"Satellite data fetch successful: {location} ({duration:.2f}s)", extra=extra_data)
    else:
        logger.error(f"Satellite data fetch failed: {location} ({duration:.2f}s)", extra=extra_data) 