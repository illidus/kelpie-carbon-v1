...
class SKEMAProcessor:
    ...
    def calculate_spectral_indices(
        ...
        if nir_array is not None and red_edge_array is not None:
            with np.errstate(divide="ignore", invalid="ignore"):
                red_edge_ndvi = (nir_array - red_edge_array) / (
                    nir_array + red_edge_array + 1e-8
                )
                indices["red_edge_ndvi"] = np.nan_to_num(
                    red_edge_ndvi, nan=0.0, posinf=0.0, neginf=0.0
                )

        return indices

    def get_kelp_probability_mask(
        self,
        rgb_array: np.ndarray,
        nir_array: np.ndarray | None = None,
        red_edge_array: np.ndarray | None = None,
        ...
        if "ndvi" in indices:
            kelp_mask |= (
                indices["ndvi"] > self.config["kelp_detection"]["ndvi_threshold"]
            )

        # NDRE threshold (if available) - primary indicator
        if "ndre" in indices:
            kelp_mask |= (
                indices["ndre"] > self.config["kelp_detection"]["ndre_threshold"]
            )
    ...
    def validate_input_bands(
        ...
        validation["spectral_guidance_possible"] = validation["rgb_valid"] and (
