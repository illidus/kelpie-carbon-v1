...
class BudgetSAMKelpDetector:
    ...
    def _find_peaks_scipy(
        self, binary_mask: np.ndarray, min_distance: int = 20
    ...
    def _generate_spectral_guidance_points(
        self,
        rgb_image: np.ndarray,
        nir_band: np.ndarray | None = None,
        red_edge_band: np.ndarray | None = None,
        ...
        guidance_points = [
            (int(x), int(y)) for y, x in zip(peaks[0], peaks[1], strict=False)
        ]

        print(f"🎯 Generated {len(guidance_points)} spectral guidance points")
        return guidance_points

    def _generate_rgb_guidance_points(
        ...
        vegetation_mask = (
            ...
            & (v >= 20)
            & (v <= 200)  # Reasonable brightness
        )

        # Find peaks
        if SKIMAGE_AVAILABLE:
            peaks = peak_local_maxima(
                vegetation_mask.astype(float), min_distance=30, threshold_abs=0.5
            )
        else:
            # Fallback using scipy
