...
try:
    ...
    SMP_AVAILABLE = False
    print("⚠️  segmentation-models-pytorch not available. Install for full U-Net functionality:")
...
class BudgetUNetKelpDetector:
    ...
    def detect_kelp(self, 
        ...
        metadata = {
            ...
            "spectral_guidance_used": self.use_spectral_guidance and (nir_band is not None or red_edge_band is not None),
        }
    ...
    def _detect_with_classical_segmentation(self, 
                                          rgb_image: np.ndarray,
                                          nir_band: np.ndarray | None = None,
                                          red_edge_band: np.ndarray | None = None) -> np.ndarray:
        """Fallback classical segmentation with spectral guidance."""
        print("🔄 Using classical segmentation fallback...")
        
        # Use spectral guidance if available
        if (self.use_spectral_guidance and self.skema_processor is not None and 
            (nir_band is not None or red_edge_band is not None)):
    ...
    def _detect_rgb_only(self, rgb_image: np.ndarray) -> np.ndarray:
        ...
        kelp_mask = (
            ...
            (v >= 30) & (v <= 220)     # Reasonable brightness
        )
        
        # Morphological cleanup
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
        kelp_mask = cv2.morphologyEx(kelp_mask.astype(np.uint8), cv2.MORPH_CLOSE, kernel)
    ...
    def create_training_data(self, 
        ...
        for i, (img_path, mask_path) in enumerate(zip(satellite_images, mask_images, strict=False)):
            # Load and preprocess
        ...
        print(f"✅ Training data created: {len(train_data)} train, {len(val_data)} val samples")
...
'''
    ...
    def batch_process_directory(self, 
        ...
        for image_file in input_path.glob(pattern):
            try:
                ...
                # Save results
                self._save_detection_results(kelp_mask, metadata, output_path, image_file.name)
    ...
    def _save_detection_results(self, kelp_mask: np.ndarray, metadata: dict, 
        ...
        plt.title(f"Budget U-Net Kelp Detection\n{metadata['kelp_pixels']:,} pixels detected")
        plt.axis('off')
        plt.tight_layout()
        plt.savefig(output_path / f"{base_name}_kelp_detection.png", dpi=150, bbox_inches='tight')
        plt.close()
