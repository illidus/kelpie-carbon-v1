...
class SKEMACNNModel(nn.Module):
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized SKEMA CNN with {num_classes} classes, "
            ...
            with torch.no_grad():
                if self.input_channels > 3:
                    ...
                    for i in range(3, self.input_channels):
                        resnet.conv1.weight[:, i] = original_conv.weight[
                            :, 1
                        ]  # Green channel
                else:
                    # Handle case with fewer than 3 channels
                    resnet.conv1.weight[:, : self.input_channels] = (
...
class MaskRCNNKelpDetector(nn.Module):
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized Mask R-CNN for kelp detection with {num_classes} classes"
        )
    ...
    def predict(
        self, images: list[torch.Tensor], confidence_threshold: float = 0.5
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (images, targets) in enumerate(train_loader):
            ...
            cls_loss = classification_criterion(
                outputs["classification"], classification_targets
            )
            seg_loss = segmentation_criterion(
                outputs["segmentation"], segmentation_targets
            )
...
def load_pretrained_skema_model(
    model_path: str, model_type: str = "skema_cnn", device: str = "cuda"
) -> nn.Module:
