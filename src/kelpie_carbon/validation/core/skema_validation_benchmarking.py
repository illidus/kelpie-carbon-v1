...
def calculate_confidence_interval(estimate, method_std, data_std):
    ...
    def _assess_mathematical_equivalence(
        ...
        # Check for parameter equivalence
            ...
            len(set(skema.parameters.keys()).intersection(set(ours.parameters.keys())))
            > 0
        ):
            common_params = len(
                set(skema.parameters.keys()).intersection(set(ours.parameters.keys()))
            )
            total_params = len(
                set(skema.parameters.keys()).union(set(ours.parameters.keys()))
            )
            equivalence_score = common_params / total_params
            equivalence_notes.append(
                f"Parameter overlap: {common_params}/{total_params}"
            )
        ...
        if hasattr(ours, "validation_range") and skema.expected_range:
            ...
            overlap = (
                max(our_range[0], skema_range[0]),
                min(our_range[1], skema_range[1]),
            )
            if overlap[0] <= overlap[1]:
                range_overlap = (overlap[1] - overlap[0]) / (
                    max(our_range[1], skema_range[1])
                    - min(our_range[0], skema_range[0])
                )
...
class VisualProcessingDemonstrator:
    ...
    def demonstrate_processing_pipeline(
        ...
        skema_detection = self._skema_final_detection(
            skema_ndre, waf_result, derivative_result
        )
        demo_results["processing_steps"].append(
            {
                "step": 5,
        ...
        our_detection = self._our_final_detection(
            our_ndre, waf_result, derivative_result
        )
        demo_results["processing_steps"].append(
            {
                "step": 6,
    ...
    def _calculate_our_ndre(self, imagery: dict[str, np.ndarray]) -> np.ndarray:
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            ndre[valid_mask] = (nir[valid_mask] - red_edge[valid_mask]) / (
                nir[valid_mask] + red_edge[valid_mask]
            )
    ...
    def _apply_water_anomaly_filter(
    ...
    def _calculate_spectral_derivatives(
        self, imagery: dict[str, np.ndarray]
    ) -> dict[str, np.ndarray]:
    ...
    def _our_final_detection(
        ...
        derivative_norm = np.clip(
            derivative_result["first_derivative"] * 1000, 0, 1
        )  # Scale derivatives
    ...
    def _calculate_performance_comparison(
        ...
        return {
            ...
            "performance_comparison": {
                "accuracy_difference": our_metrics["accuracy"]
                - skema_metrics["accuracy"],
                "f1_difference": our_metrics["f1_score"] - skema_metrics["f1_score"],
                "precision_difference": our_metrics["precision"]
                - skema_metrics["precision"],
    ...
    def create_visualization_figure(self, demo_results: dict[str, Any]) -> plt.Figure:
        ...
        axes[0, 0].imshow(
            np.stack(
                [
                    demo_results["original_imagery"]["red"],
                    demo_results["original_imagery"]["green"],
                    demo_results["original_imagery"]["blue"],
                ],
                axis=2,
            )
        )
        ...
        # Ground truth
        axes[0, 1].imshow(
            demo_results["final_results"]["ground_truth"], cmap="RdYlGn", alpha=0.8
        )
        ...
        # SKEMA NDRE
        im1 = axes[0, 2].imshow(
            demo_results["processing_steps"][0]["result"],
            cmap="RdYlGn",
            vmin=-0.5,
            vmax=0.5,
        )
        ...
        # Our NDRE
        im2 = axes[0, 3].imshow(
            demo_results["processing_steps"][1]["result"],
            cmap="RdYlGn",
            vmin=-0.5,
            vmax=0.5,
        )
        ...
        axes[1, 2].imshow(
            demo_results["final_results"]["skema_detection"], cmap="RdYlGn", alpha=0.8
        )
        ...
        accuracy_values = [
            metrics["skema_metrics"]["accuracy"],
            metrics["our_metrics"]["accuracy"],
        ]
        f1_values = [
            metrics["skema_metrics"]["f1_score"],
            metrics["our_metrics"]["f1_score"],
        ]
        ...
        axes[2, 0].bar(
            x - width / 2, accuracy_values, width, label="Accuracy", alpha=0.8
        )
        ...
        our_flat = demo_results["final_results"]["our_detection"][
            "final_detection"
        ].flatten()
