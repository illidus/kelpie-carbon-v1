...
class Phase3DataAcquisition:
    ...
    def __init__(self, data_directory: str | None = None):
        ...
        logger.info(
            f"Initialized Phase3DataAcquisition with {len(self.validation_sites)} sites"
        )
    ...
    def get_validation_sites(
        ...
        if confidence:
            sites = [
                s
                for s in sites
                if s.validation_confidence.lower() == confidence.lower()
            ]

        logger.info(
            f"Found {len(sites)} sites matching filters: region={region}, species={species}, confidence={confidence}"
        )
        return sites
    ...
    def create_synthetic_sentinel2_scenes(
        ...
        logger.info(
            f"Creating {num_scenes} synthetic Sentinel-2 scenes for {site.name}"
        )
        ...
        for i in range(num_scenes):
            ...
            scene_date = start_date + datetime.timedelta(
                days=date_fraction * (end_date - start_date).days
            )
        ...
        logger.info(
            f"Created {len(scenes)} synthetic scenes with avg {np.mean([s.cloud_coverage for s in scenes]):.1f}% cloud coverage"
        )
        return scenes
    ...
    def create_validation_dataset(
        ...
        logger.info(
            f"Created validation dataset for {site.name} with {len(scenes)} scenes, quality={quality_metrics['overall_quality']:.2f}"
        )
        return dataset
    ...
    def _calculate_quality_metrics(
        ...
        good_or_better_pct = (
            (quality_counts["excellent"] + quality_counts["good"]) / total_scenes * 100
        )
        usable_pct = (
            (
        ...
        if len(dates) > 2:
            ...
            temporal_uniformity = (
                1.0 - (np.std(date_diffs) / np.mean(date_diffs))
                if np.mean(date_diffs) > 0
                else 0.0
            )
        ...
        seasonal_coverage = (
            unique_phases / 4.0
        )  # 4 possible phases (early, mid, peak, late)
        ...
        return {
            ...
            "usable_scenes": quality_counts["excellent"]
            + quality_counts["good"]
            + quality_counts["fair"],
    ...
    def create_benchmark_suite(
        ...
        logger.info(
            f"Creating benchmark suite for {len(sites)} sites with {num_scenes_per_site} scenes each"
        )
        ...
        for site_id in sites:
            ...
            try:
                dataset = self.create_validation_dataset(
                    site_id, num_scenes_per_site, year
                )
        ...
        logger.info(
            f"Created benchmark suite with {len(benchmark_suite)} datasets, {total_scenes} total scenes, avg quality={avg_quality:.2f}"
        )
    ...
    def load_validation_dataset(self, filepath: str) -> ValidationDataset:
        ...
        dataset = ValidationDataset(
            ...
            created_date=datetime.datetime.fromisoformat(
                dataset_dict["metadata"]["created_date"]
            ),
        )

        logger.info(
            f"Loaded validation dataset from {filepath} with {len(scenes)} scenes"
        )
        return dataset
    ...
    def get_site_summary(self) -> dict[str, Any]:
        ...
        for site in self.validation_sites.values():
            ...
            region = site.region
            summary["sites_by_region"][region] = (
                summary["sites_by_region"].get(region, 0) + 1
            )

            # Species distribution
            species = site.species
            summary["sites_by_species"][species] = (
                summary["sites_by_species"].get(species, 0) + 1
            )

            # Confidence distribution
            confidence = site.validation_confidence
            summary["sites_by_confidence"][confidence] = (
                summary["sites_by_confidence"].get(confidence, 0) + 1
            )
            ...
            summary["coordinate_bounds"]["min_lat"] = min(
                summary["coordinate_bounds"]["min_lat"], lat
            )
            summary["coordinate_bounds"]["max_lat"] = max(
                summary["coordinate_bounds"]["max_lat"], lat
            )
            summary["coordinate_bounds"]["min_lon"] = min(
                summary["coordinate_bounds"]["min_lon"], lon
            )
            summary["coordinate_bounds"]["max_lon"] = max(
                summary["coordinate_bounds"]["max_lon"], lon
            )
    ...
    def generate_quality_report(
        ...
        for site_id, dataset in datasets.items():
            ...
            all_cloud_coverages.append(
                dataset.quality_metrics["average_cloud_coverage"]
            )
            ...
            report["site_quality"][site_id] = {
                ...
                "average_cloud_coverage": dataset.quality_metrics[
                    "average_cloud_coverage"
                ],
                "temporal_span_days": dataset.quality_metrics["temporal_span_days"],
                "recommendation": self._get_site_recommendation(
        ...
        report["overall_quality"] = {
            ...
            "total_usable_scenes": sum(
                d.quality_metrics["usable_scenes"] for d in datasets.values()
            ),
        }
        ...
        if avg_quality >= 0.8:
            report["recommendations"].append(
                "Dataset quality is excellent for validation purposes"
            )
        elif avg_quality >= 0.6:
            report["recommendations"].append(
                "Dataset quality is good, suitable for most validation tasks"
            )
        else:
            report["recommendations"].append(
                "Dataset quality could be improved with additional high-quality scenes"
            )

        avg_cloud = report["overall_quality"]["average_cloud_coverage"]
        if avg_cloud > 50:
            report["recommendations"].append(
                "Consider filtering scenes with >50% cloud coverage for clearer results"
            )
        ...
        if poor_scenes / total_scenes > 0.3:
            report["recommendations"].append(
                "High proportion of poor-quality scenes - consider regenerating with stricter quality criteria"
            )
