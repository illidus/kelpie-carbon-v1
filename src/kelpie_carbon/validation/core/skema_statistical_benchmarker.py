...
class StatisticalBenchmarker:
    ...
    def _analyze_single_site(self, site_data: dict[str, Any]) -> BenchmarkComparison:
        ...
        skema_metrics = self._calculate_performance_metrics(
            skema_predictions, ground_truth
        )
    ...
    def _perform_statistical_tests(
        ...
        w_stat, w_pval = stats.wilcoxon(
            skema_errors, our_errors, alternative="two-sided"
        )
    ...
    def _mcnemar_test(
        ...
        if skema_only + our_only == 0:
            ...
            mcnemar_stat = (abs(skema_only - our_only) - 1) ** 2 / (
                skema_only + our_only
            )
            p_value = 1 - stats.chi2.cdf(mcnemar_stat, 1)

        effect_size = (
            abs(skema_only - our_only) / (skema_only + our_only)
            if (skema_only + our_only) > 0
            else 0
        )
    ...
    def _perform_bootstrap_analysis(
        ...
        return {"skema": skema_bootstrap, "ours": our_bootstrap}
    ...
    def _recommend_mcnemar(self, p_value: float, skema_only: int, our_only: int) -> str:
        ...
        if p_value < self.alpha:
            if our_only > skema_only:
                return (
                    "Our pipeline shows significantly better classification performance"
                )
            else:
    ...
    def _generate_overall_assessment(
        ...
        significant_tests = [
            test for test in statistical_tests if test.p_value < self.alpha
        ]
        ...
        if f1_diff > 0.05:
            assessment_parts.append(
                "Our pipeline shows better balanced performance (F1)"
            )
        elif f1_diff < -0.05:
        ...
        if len(significant_tests) > 0:
            assessment_parts.append(
                f"{len(significant_tests)} statistical tests show significant differences"
            )
        else:
    ...
    def _perform_meta_analysis(
        ...
        meta_results = {
            ...
            "accuracy_effect_size": (
                np.mean(our_accuracies) - np.mean(skema_accuracies)
            )
            / np.sqrt(
                (np.std(our_accuracies) ** 2 + np.std(skema_accuracies) ** 2) / 2
            ),
            "sites_favoring_ours": sum(
                1
                for comp in comparisons
                if comp.our_results["accuracy"] > comp.skema_results["accuracy"]
            ),
            "sites_favoring_skema": sum(
                1
                for comp in comparisons
                if comp.skema_results["accuracy"] > comp.our_results["accuracy"]
            ),
        }
    ...
    def _print_site_summary(self, comparison: BenchmarkComparison) -> None:
        ...
        print(
            f"   🚀 Ours:  Accuracy={comparison.our_results['accuracy']:.3f}, F1={comparison.our_results['f1_score']:.3f}"
        )
        print(f"   🔍 Assessment: {comparison.overall_assessment}")

        # Show significant tests
        significant_tests = [
            test for test in comparison.statistical_tests if test.p_value < self.alpha
        ]
    ...
    def _print_meta_analysis(self, meta: dict[str, Any]) -> None:
        ...
        print(
            f"   📊 SKEMA average accuracy: {meta['skema_mean_accuracy']:.3f} ± {meta['skema_std_accuracy']:.3f}"
        )
        print(
            f"   🚀 Our average accuracy: {meta['our_mean_accuracy']:.3f} ± {meta['our_std_accuracy']:.3f}"
        )
        print(f"   📈 Effect size (accuracy): {meta['accuracy_effect_size']:.3f}")
        print(
            f"   🏆 Sites favoring our method: {meta['sites_favoring_ours']}/{meta['n_sites']}"
        )

        if "combined_p_value" in meta:
            print(
                f"   🔬 Combined statistical significance: p={meta['combined_p_value']:.4f}"
            )
