...
class EnhancedValidationMetrics:
    ...
    def calculate_biomass_accuracy_metrics(
        ...
        if len(predicted) != len(observed):
            logger.warning(
                f"Mismatched array lengths: pred={len(predicted)}, obs={len(observed)}"
            )
        ...
        if len(predicted_clean) == 1:
            ...
            return {
                ...
                "mape_percentage": (
                    float(abs(residual / observed_clean[0] * 100))
                    if observed_clean[0] != 0
                    else 0.0
                ),
                "bias_percentage": (
                    float(residual / observed_clean[0] * 100)
                    if observed_clean[0] != 0
                    else 0.0
                ),
                "uncertainty_bounds_95": {
                    "lower_bound": 0.0,
                    "upper_bound": 0.0,
        ...
        try:
            ...
            mape = (
                np.mean(
                    np.abs(
                        (observed_clean - predicted_clean)
                        / np.where(observed_clean != 0, observed_clean, 1)
                    )
                )
                * 100
            )
            bias_percentage = (
                np.mean(
                    (predicted_clean - observed_clean)
                    / np.where(observed_clean != 0, observed_clean, 1)
                )
                * 100
            )
            ...
            uncertainty_bounds_95 = self._calculate_prediction_intervals(
                predicted_clean, observed_clean, 0.95
            )
    ...
    def calculate_carbon_accuracy_metrics(
        ...
        try:
            ...
            rmse_carbon = (
                np.sqrt(mean_squared_error(carbon_obs_clean, carbon_pred_clean)) * 10
            )  # Convert to tC/hectare
            ...
            sequestration_rate_rmse = self._calculate_annual_sequestration_rmse(
                carbon_pred_clean, carbon_obs_clean
            )
            carbon_bias_percentage = (
                np.mean(
                    (carbon_pred_clean - carbon_obs_clean)
                    / np.where(carbon_obs_clean != 0, carbon_obs_clean, 1)
                )
                * 100
            )

            # Statistical metrics for carbon
            carbon_pearson_corr, carbon_pearson_p = stats.pearsonr(
                carbon_pred_clean, carbon_obs_clean
            )
        ...
        for data in validation_data:
            ...
            try:
                ...
                biomass_metrics = self.calculate_biomass_accuracy_metrics(
                    data.predicted_biomass_kg_m2, data.observed_biomass_kg_m2
                )

                # Calculate carbon metrics
                carbon_factors = {
                    "carbon_content_ratio": self.SPECIES_CARBON_RATIOS.get(
                        data.coordinate.species, self.SPECIES_CARBON_RATIOS["Mixed"]
                    )
                }
                ...
                species_metrics = self._calculate_species_specific_metrics(
                    data.coordinate.species, biomass_metrics, carbon_metrics
                )
                ...
                logger.info(
                    f"Validation complete for {data.coordinate.name}: "
    ...
    def generate_validation_summary(
        ...
        overall_performance = {
            "biomass_metrics": {
                ...
                "mean_rmse_kg_m2": (
                    float(np.mean(all_rmse_biomass)) if all_rmse_biomass else 0.0
                ),
                "mean_mae_kg_m2": (
                    float(np.mean(all_mae_biomass)) if all_mae_biomass else 0.0
                ),
            ...
            "carbon_metrics": {
                ...
                "mean_rmse_tc_hectare": (
                    float(np.mean(all_rmse_carbon)) if all_rmse_carbon else 0.0
                ),
                "mean_mae_tc_hectare": (
                    float(np.mean(all_mae_carbon)) if all_mae_carbon else 0.0
                ),
        ...
        for species, data in species_performance.items():
            species_summary[species] = {
                ...
                "sites": data["sites"],
                "mean_biomass_r2": (
                    float(np.mean(data["biomass_r2_values"]))
                    if data["biomass_r2_values"]
                    else 0.0
                ),
                "mean_carbon_r2": (
                    float(np.mean(data["carbon_r2_values"]))
                    if data["carbon_r2_values"]
                    else 0.0
                ),
            }
    ...
    def _calculate_prediction_intervals(
        self, predicted: np.ndarray, observed: np.ndarray, confidence_level: float
    ) -> dict[str, float]:
    ...
    def _calculate_annual_sequestration_rmse(
        self, carbon_pred: np.ndarray, carbon_obs: np.ndarray
    ) -> float:
    ...
    def _calculate_species_specific_metrics(
        ...
        try:
            carbon_ratio = self.SPECIES_CARBON_RATIOS.get(
                species, self.SPECIES_CARBON_RATIOS["Mixed"]
            )

            return {
                "species": species,
                "carbon_content_ratio": carbon_ratio,
                "biomass_performance_score": biomass_metrics.get(
                    "r2_biomass_correlation", 0
                )
                * biomass_metrics.get("pearson_correlation", 0),
                "carbon_performance_score": carbon_metrics.get(
                    "r2_carbon_correlation", 0
                )
                * carbon_metrics.get("carbon_pearson_correlation", 0),
                "overall_species_score": (
                    biomass_metrics.get("r2_biomass_correlation", 0)
                    + carbon_metrics.get("r2_carbon_correlation", 0)
                )
                / 2,
            }
        except Exception as e:
    ...
    def _calculate_temporal_metrics(
        self, measurement_dates: list[str], predicted: np.ndarray, observed: np.ndarray
    ) -> dict[str, Any]:
        ...
        try:
            ...
            monthly_performance = (
                df.groupby("month")["residual"].agg(["mean", "std"]).to_dict("index")
            )
            ...
            return {
                ...
                "temporal_correlation": float(
                    stats.spearmanr(range(len(dates)), predicted - observed)[0]
                ),
            }
        except Exception as e:
