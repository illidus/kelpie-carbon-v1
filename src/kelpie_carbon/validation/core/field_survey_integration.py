    ...
    def _convert_dataframe_to_records(
        ...
        for _, row in df.iterrows():
            try:
                ...
                observed_species = self._parse_species_list(
                    row.get("observed_species", "")
                )
                ...
                record = FieldSurveyRecord(
                    ...
                    biomass_measurement_method=str(
                        row.get("biomass_measurement_method", "visual_estimate")
                    ),
    ...
    def _convert_dict_to_record(self, data: dict[str, Any]) -> FieldSurveyRecord | None:
        ...
        try:
            return FieldSurveyRecord(
                ...
                biomass_measurement_method=data.get(
                    "biomass_measurement_method", "visual_estimate"
                ),
...
class SpeciesValidationAnalyzer:
    ...
    def _calculate_species_classification_metrics(
        ...
        correct_predictions = sum(
            1
            for pred, actual in zip(predicted_species, actual_species, strict=False)
            if pred == actual
        )
        ...
        for species in all_species:
            ...
            f1 = (
                2
                * (precision[species] * recall[species])
                / (precision[species] + recall[species])
                if (precision[species] + recall[species]) > 0
                else 0.0
            )
            f1_score[species] = f1

            samples_by_species[species] = sum(
                1 for actual in actual_species if actual == species
            )
        ...
        confidence_dist = {
            "high": sum(
                1 for pair in matched_pairs if pair[0].get("confidence", 0.0) >= 0.8
            ),
            "medium": sum(
                1
                for pair in matched_pairs
                if 0.5 <= pair[0].get("confidence", 0.0) < 0.8
            ),
            "low": sum(
                1 for pair in matched_pairs if pair[0].get("confidence", 0.0) < 0.5
            ),
        }
    ...
    def _calculate_biomass_estimation_metrics(
        ...
        biomass_pairs = [
            (pair[0], pair[1])
            for pair in matched_pairs
            if pair[0].get("biomass_estimate_kg_per_m2") is not None
            and pair[1].biomass_kg_per_m2 is not None
        ]
        ...
        predicted_biomass = np.array(
            [pair[0].get("biomass_estimate_kg_per_m2", 0.0) for pair in biomass_pairs]
        )
        ...
        for species in all_species:
            species_pairs = [
                ...
                if pair[1].primary_species == species
            ]
            if species_pairs:
                species_pred = np.array(
                    [
                        pair[0].get("biomass_estimate_kg_per_m2", 0.0)
                        for pair in species_pairs
                    ]
                )
                species_actual = np.array(
                    [pair[1].biomass_kg_per_m2 for pair in species_pairs]
                )

                by_species[species] = {
                    "mae": float(np.mean(np.abs(species_pred - species_actual))),
                    "rmse": float(
                        np.sqrt(np.mean((species_pred - species_actual) ** 2))
                    ),
        ...
        confidence_dist = {
            "high": sum(
                1 for pair in biomass_pairs if pair[1].biomass_confidence == "high"
            ),
            "moderate": sum(
                1 for pair in biomass_pairs if pair[1].biomass_confidence == "moderate"
            ),
            "low": sum(
                1 for pair in biomass_pairs if pair[1].biomass_confidence == "low"
            ),
        }
...
class FieldSurveyReporter:
    ...
    def generate_comprehensive_report(
        ...
        report = {
            ...
            "summary": self._generate_summary(
                validation_metrics, field_records, predictions
            ),
            "species_classification": self._generate_species_classification_report(
                validation_metrics
            ),
            "biomass_estimation": self._generate_biomass_estimation_report(
                validation_metrics
            ),
    ...
    def _generate_summary(
        ...
        return {
            ...
            "dominant_species_in_survey": (
                max(metrics.samples_by_species.items(), key=lambda x: x[1])[0]
                if metrics.samples_by_species
                else "None"
            ),
            "data_quality": self._assess_data_quality(metrics),
        }

    def _generate_species_classification_report(
        self, metrics: SpeciesValidationMetrics
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "per_species_performance": {
                ...
                for species in set(
                    list(metrics.species_precision.keys())
                    + list(metrics.samples_by_species.keys())
                )
            },
    ...
    def _generate_biomass_estimation_report(
        self, metrics: SpeciesValidationMetrics
    ) -> dict[str, Any]:
    ...
    def _generate_field_survey_summary(
        self, field_records: list[FieldSurveyRecord]
    ) -> dict[str, Any]:
        ...
        for record in field_records:
            ...
            species_counts[record.primary_species] = (
                species_counts.get(record.primary_species, 0) + 1
            )
        ...
        return {
            ...
            "biomass_statistics": {
                ...
                "mean_kg_per_m2": (
                    np.mean(biomass_measurements) if biomass_measurements else 0.0
                ),
                "std_kg_per_m2": (
                    np.std(biomass_measurements) if biomass_measurements else 0.0
                ),
                "min_kg_per_m2": (
                    np.min(biomass_measurements) if biomass_measurements else 0.0
                ),
                "max_kg_per_m2": (
                    np.max(biomass_measurements) if biomass_measurements else 0.0
                ),
            },
            "temporal_coverage": {
                "earliest_survey": min(
                    record.timestamp for record in field_records
                ).isoformat(),
                "latest_survey": max(
                    record.timestamp for record in field_records
                ).isoformat(),
    ...
    def _generate_model_performance_summary(
        ...
        return {
            ...
            "confidence_statistics": {
                ...
                "max_confidence": (
                    np.max(confidence_scores) if confidence_scores else 0.0
                ),
            },
            "biomass_prediction_statistics": {
                "predictions_count": len(biomass_estimates),
                "mean_kg_per_m2": (
                    np.mean(biomass_estimates) if biomass_estimates else 0.0
                ),
                "std_kg_per_m2": (
                    np.std(biomass_estimates) if biomass_estimates else 0.0
                ),
                "min_kg_per_m2": (
                    np.min(biomass_estimates) if biomass_estimates else 0.0
                ),
                "max_kg_per_m2": (
                    np.max(biomass_estimates) if biomass_estimates else 0.0
                ),
            },
        }
    ...
    def _generate_recommendations(self, metrics: SpeciesValidationMetrics) -> list[str]:
        ...
        if metrics.species_accuracy < 0.7:
            recommendations.append(
                "Species classification accuracy is below 70%. Consider collecting more training data or adjusting classification thresholds."
            )

        if metrics.biomass_mae > 5.0:
            recommendations.append(
                "Biomass estimation error is high (>5 kg/m²). Review biomass calculation parameters and consider site-specific calibration."
            )

        if metrics.biomass_r2 < 0.5:
            recommendations.append(
                "Low correlation between predicted and observed biomass. Investigate morphological feature extraction and enhancement factors."
            )
        ...
        for species, precision in metrics.species_precision.items():
            if precision < 0.6 and metrics.samples_by_species.get(species, 0) > 5:
                recommendations.append(
                    f"Low precision for {species} classification. Consider species-specific model tuning."
                )

        if not recommendations:
            recommendations.append(
                "Validation results meet quality standards. Continue monitoring with periodic validation campaigns."
            )
    ...
    def process_field_survey_campaign(
        ...
        try:
            ...
            report = self.reporter.generate_comprehensive_report(
                ...
                model_predictions,
                (
                    output_dir / f"{campaign_id}_validation_report.json"
                    if output_dir
                    else None
                ),
            )
            ...
            self._store_validation_results(
                campaign_id, validation_metrics, field_records
            )

            self.logger.info(
                f"Campaign processing complete. Overall accuracy: {validation_metrics.species_accuracy:.1%}"
            )
