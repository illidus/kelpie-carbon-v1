...
class ValidationVisualizationSuite:
    ...
    def __init__(self, output_dir: str = "validation_plots"):
        ...
        logger.info(
            f"Validation visualization suite initialized. Output: {self.output_dir}"
        )
    ...
    def plot_rmse_mae_r2_comparison(self, validation_results: dict) -> str:
        ...
        fig.suptitle(
            "Model Accuracy Metrics Comparison", fontsize=16, fontweight="bold"
        )
    ...
    def visualize_spatial_accuracy_distribution(self, validation_results: dict) -> str:
        ...
        if not FOLIUM_AVAILABLE:
            logger.warning(
                "Folium not available - creating static spatial plot instead"
            )
    ...
    def create_species_accuracy_comparison(self, validation_results: dict) -> str:
        ...
        for site_name, result in validation_results.items():
            ...
            species_data[species]["biomass_r2"].append(
                result.biomass_metrics.get("r2_biomass_correlation", 0)
            )
            species_data[species]["biomass_rmse"].append(
                result.biomass_metrics.get("rmse_biomass_kg_m2", 0)
            )
            species_data[species]["carbon_r2"].append(
                result.carbon_metrics.get("r2_carbon_correlation", 0)
            )
            species_data[species]["carbon_rmse"].append(
                result.carbon_metrics.get("rmse_carbon_tc_hectare", 0)
            )
        ...
        # Biomass R²
        mean_biomass_r2 = [
            np.mean(species_data[sp]["biomass_r2"]) for sp in species_names
        ]
        std_biomass_r2 = [
            np.std(species_data[sp]["biomass_r2"]) for sp in species_names
        ]
        ...
        axes[0, 0].bar(
            ...
            mean_biomass_r2,
            yerr=std_biomass_r2,
            color=colors[: len(species_names)],
            alpha=0.7,
            capsize=5,
        )
        ...
        # Biomass RMSE
        mean_biomass_rmse = [
            np.mean(species_data[sp]["biomass_rmse"]) for sp in species_names
        ]
        std_biomass_rmse = [
            np.std(species_data[sp]["biomass_rmse"]) for sp in species_names
        ]

        axes[0, 1].bar(
            species_names,
            mean_biomass_rmse,
            yerr=std_biomass_rmse,
            color=colors[: len(species_names)],
            alpha=0.7,
            capsize=5,
        )
        ...
        # Carbon R²
        mean_carbon_r2 = [
            np.mean(species_data[sp]["carbon_r2"]) for sp in species_names
        ]
        ...
        # Carbon RMSE
        mean_carbon_rmse = [
            np.mean(species_data[sp]["carbon_rmse"]) for sp in species_names
        ]
        std_carbon_rmse = [
            np.std(species_data[sp]["carbon_rmse"]) for sp in species_names
        ]

        axes[1, 1].bar(
            species_names,
            mean_carbon_rmse,
            yerr=std_carbon_rmse,
            color=colors[: len(species_names)],
            alpha=0.7,
            capsize=5,
        )
    ...
    def plot_temporal_accuracy_trends(self, temporal_data: dict) -> str:
        ...
        accuracy_trend = 0.8 + 0.15 * np.sin(
            np.array(months) * 2 * np.pi / 12 - np.pi / 2
        )
    ...
    def create_uncertainty_calibration_plots(
        self,
        ...
        for bin_lower, bin_upper in zip(bin_lowers, bin_uppers, strict=False):
            ...
            if in_bin.sum() > 0:
                accuracy_in_bin = (
                    1
                    - np.abs(predictions[in_bin] - observations[in_bin])
                    / observations[in_bin]
                )
        ...
        for level in coverage_levels:
            ...
            within_interval = (observations >= lower_bound) & (
                observations <= upper_bound
            )
    ...
    def generate_validation_report_visualizations(
        ...
        try:
            ...
            if "validation_results" in all_metrics:
                ...
                # Core accuracy plots
                plot_paths["accuracy_dashboard"] = (
                    self.create_accuracy_assessment_dashboard(validation_results)
                )
                plot_paths["rmse_mae_r2_comparison"] = self.plot_rmse_mae_r2_comparison(
                    validation_results
                )

                # Scatter plots
                scatter_plots = self.create_predicted_vs_actual_plots(
                    validation_results
                )
                plot_paths.update(scatter_plots)

                # Spatial and species plots
                plot_paths["spatial_accuracy"] = (
                    self.visualize_spatial_accuracy_distribution(validation_results)
                )
                plot_paths["species_comparison"] = (
                ...
                if "temporal_data" in all_metrics:
                    plot_paths["temporal_trends"] = self.plot_temporal_accuracy_trends(
                        all_metrics["temporal_data"]
                    )
            ...
            plot_paths["validation_summary"] = self._create_validation_summary_plot(
                all_metrics
            )

            logger.info(
                f"Complete validation report visualizations generated: {len(plot_paths)} plots"
            )
    ...
    def _create_validation_summary_plot(self, all_metrics: dict) -> str:
        ...
        if "validation_summary" in all_metrics:
            ...
            summary_text += (
                f"Total Sites Validated: {vs.get('total_sites_validated', 'N/A')}\n"
            )
            ...
            if "overall_performance" in vs:
                ...
                summary_text += (
                    f"  Mean RMSE: {biomass.get('mean_rmse_kg_m2', 0):.3f} kg/m²\n"
                )
...
def create_accuracy_assessment_dashboard(
    validation_results: dict, output_dir: str = "validation_plots"
) -> str:
...
def plot_rmse_mae_r2_comparison(
    validation_results: dict, output_dir: str = "validation_plots"
) -> str:
...
def visualize_spatial_accuracy_distribution(
    validation_results: dict, output_dir: str = "validation_plots"
) -> str:
