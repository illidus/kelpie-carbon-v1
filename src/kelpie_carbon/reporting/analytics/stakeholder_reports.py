...
class FirstNationsReport(BaseStakeholderReport):
    ...
    def generate_content(self, analysis_result: AnalysisResult) -> dict[str, Any]:
        ...
        content = {
            ...
            "traditional_knowledge": self._create_traditional_knowledge_section(
                analysis_result
            ),
            "stewardship_recommendations": self._create_stewardship_recommendations(
                analysis_result
            ),
            "monitoring_partnership": self._create_monitoring_partnership(
                analysis_result
            ),
    ...
    def get_key_messages(self, analysis_result: AnalysisResult) -> list[str]:
        ...
        if "kelp_extent" in analysis_result.results.get("validation", {}):
            ...
            messages.append(
                f"Current kelp forests cover {extent:.1f} hectares in this area"
            )
        ...
        if "historical" in analysis_result.results:
            ...
            if "current_vs_historical" in historical:
                ...
                if change < 0:
                    messages.append(
                        f"Kelp extent has decreased by {abs(change):.1f} hectares since historical times"
                    )
                else:
                    messages.append(
                        f"Kelp extent has increased by {change:.1f} hectares since historical times"
                    )
        ...
        if overall_confidence > 0.8:
            messages.append(
                "Kelp forest monitoring shows good data quality and confidence"
            )
        elif overall_confidence > 0.6:
            messages.append(
                "Kelp forest monitoring shows moderate confidence - more data recommended"
            )
        else:
            messages.append(
                "Kelp forest monitoring shows lower confidence - enhanced monitoring needed"
            )
    ...
    def _create_traditional_knowledge_section(
    ...
    def _create_stewardship_recommendations(
        self, analysis_result: AnalysisResult
    ) -> str:
...
class ScientificReport(BaseStakeholderReport):
    ...
    def get_key_messages(self, analysis_result: AnalysisResult) -> list[str]:
        ...
        if "validation" in analysis_result.results:
            ...
            if "kelp_extent" in validation and "detection_accuracy" in validation:
                ...
                messages.append(
                    f"Kelp extent: {extent:.1f} ± {extent * 0.1:.1f} ha (detection accuracy: {accuracy:.1%})"
                )
    ...
    def _create_results_section(self, analysis_result: AnalysisResult) -> str:
        ...
        if analysis_result.confidence_scores:
            results.extend(["Confidence Assessment:", ""])
        ...
        if analysis_result.metrics:
            results.extend(["Performance Metrics:", ""])
    ...
    def _create_uncertainty_analysis(self, analysis_result: AnalysisResult) -> str:
        ...
        if analysis_result.uncertainty_estimates:
            ...
            for analysis_type, (
                lower,
                upper,
            ) in analysis_result.uncertainty_estimates.items():
        ...
        if analysis_result.data_quality:
            uncertainty.extend(["Data Quality Assessment:", ""])
...
class ManagementReport(BaseStakeholderReport):
    ...
    def generate_content(self, analysis_result: AnalysisResult) -> dict[str, Any]:
        ...
        content = {
            ...
            "management_recommendations": self._create_management_recommendations(
                analysis_result
            ),
            "monitoring_strategy": self._create_monitoring_strategy(analysis_result),
            "resource_requirements": self._create_resource_requirements(
                analysis_result
            ),
    ...
    def _create_executive_dashboard(self, analysis_result: AnalysisResult) -> str:
        ...
        if "historical" in analysis_result.results:
            ...
            dashboard.extend([f"RISK LEVEL: {risk_level}", ""])
    ...
    def _create_status_assessment(self, analysis_result: AnalysisResult) -> str:
        ...
        if "historical" in analysis_result.results:
            ...
            if "current_vs_historical" in historical:
                ...
                if change > 0:
                    status.append(
                        f"• Change from Historical: +{change:.1f} ha (increase)"
                    )
                else:
                    status.append(
                        f"• Change from Historical: {change:.1f} ha (decrease)"
                    )
    ...
    def _create_risk_analysis(self, analysis_result: AnalysisResult) -> str:
        ...
        if "historical" in analysis_result.results:
            ...
            risk.extend(
                [f"Overall Risk Level: {risk_level}", "", "Identified Risk Factors:"]
            )
    ...
    def _create_management_recommendations(
        self, analysis_result: AnalysisResult
    ) -> str:
    ...
    def _create_monitoring_strategy(self, analysis_result: AnalysisResult) -> str:
        ...
        if overall_confidence < 0.7:
            ...
            "historical" in analysis_result.results
            and analysis_result.results["historical"].get("risk_level") == "HIGH"
        ):
            frequency = "Quarterly"
    ...
    def _create_resource_requirements(self, analysis_result: AnalysisResult) -> str:
        ...
        if "historical" in analysis_result.results:
            risk_level = analysis_result.results["historical"].get(
                "risk_level", "UNKNOWN"
            )
