    ...
    def analyze_test_files_for_removal(self):
        ...
        for test_file in self.test_root.rglob("test_*.py"):
            ...
            try:
                ...
                validation_tests = len(re.findall(r'def test.*validation|def test.*invalid|def test.*error', content, re.IGNORECASE))
                creation_tests = len(re.findall(r'def test.*creation|def test.*create|def test_valid', content, re.IGNORECASE))
                edge_case_tests = len(re.findall(r'def test.*edge|def test.*empty|def test.*none|def test.*missing', content, re.IGNORECASE))
                ...
                is_redundant = (
                    (validation_ratio > 0.6 and validation_tests > 5) or
                    (creation_ratio > 0.7 and creation_tests > 3) or
                    (edge_ratio > 0.5 and edge_case_tests > 4) or
                    (validation_ratio + creation_ratio + edge_ratio > 0.8)
                )
                ...
                if is_redundant:
                    removal_candidates.append({
                        ...
                        'redundancy_score': validation_ratio + creation_ratio + edge_ratio
                    })
        ...
        for i, candidate in enumerate(removal_candidates[:10]):
            print(f"  {i+1}. {candidate['file'].name}: {candidate['total_tests']} tests, "
...
'''
    ...
    def remove_redundant_files(self, dry_run=False):
        ...
        candidates_to_remove = [
            candidate for candidate in self.files_to_remove 
            if candidate['redundancy_score'] > 0.7 and candidate['total_tests'] > 3
        ]
        
        for candidate in candidates_to_remove[:20]:  # Remove up to 20 most redundant files
            file_path = candidate['file']
            
            # Skip critical files (those with integration or e2e in path)
            if any(critical in str(file_path) for critical in ['integration', 'e2e', 'conftest']):
                continue
    ...
    def consolidate_similar_test_classes(self):
        ...
        for test_file in self.test_root.rglob("test_*.py"):
            ...
            try:
                ...
                for class_name in classes:
                    ...
                    base_name = re.sub(r'Test|Analysis|Validation|Framework', '', class_name)
                    if base_name:
        ...
        print(f"Found {len(consolidation_opportunities)} class consolidation opportunities")
    ...
    def generate_consolidation_report(self):
        ...
        report = {
            ...
            'estimated_test_reduction': sum(c['total_tests'] for c in self.files_to_remove),
    ...
    def run_aggressive_consolidation(self, dry_run=False):
        ...
        print(f"   • Parameterized files created: {report['parameterized_files_created']}")
        
        reduction_percentage = (len(removed_files) / report['original_file_count']) * 100
