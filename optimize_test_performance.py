...
class TestPerformanceOptimizer:
    ...
    def parse_durations(self, filepath: str) -> Dict[str, float]:
        ...
        duration_section = re.search(r'slowest \d+ durations.*?====', content, re.DOTALL)
...
'''
    ...
    def mark_slow_tests(self, heavy_tests: List[str]):
        ...
        for file_path in slow_test_files:
            ...
            for i, line in enumerate(lines):
                ...
                if (line.strip().startswith('class Test') and 
                    any(test_path.split('::')[1] in line for test_path in heavy_tests 
                        if test_path.startswith(file_path))):
                    modified_lines.append('@pytest.mark.slow')
                    
                # Add slow marker before individual test functions
                elif (line.strip().startswith('def test_') and 
                      any(test_path.split('::')[-1] in line for test_path in heavy_tests 
                          if test_path.startswith(file_path))):
                    modified_lines.append('@pytest.mark.slow')
    ...
    def optimize_heavy_fixture_usage(self, heavy_tests: List[str]):
        ...
        replacements = [
            ...
            (r'await asyncio\.sleep\([^)]+\)', '# await asyncio.sleep removed for performance'),
    ...
    def _add_fixture_dependencies(self, content: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if line.strip().startswith('def test_') and '(' in line:
                ...
                if 'sample_historical_dataset' in content and 'sample_historical_dataset' not in line:
                ...
                if 'mock_network_requests' in content and 'mock_network_requests' not in line:
    ...
    def install_pytest_xdist(self):
        ...
        try:
            subprocess.run(['poetry', 'add', '--group', 'dev', 'pytest-xdist'], 
                          check=True, capture_output=True)
            print("✅ Installed pytest-xdist for parallel execution")
        except subprocess.CalledProcessError as e:
            print(f"❌ Failed to install pytest-xdist: {e}")
    
    def create_performance_report(self, before_durations: Dict[str, float], 
                                after_durations: Dict[str, float]):
...
"""
        ...
        for test_path, before, after, improvement in improvements[:10]:
            report += f"- `{test_path}`: {before:.1f}s → {after:.1f}s (-{improvement:.1f}s)\n"
