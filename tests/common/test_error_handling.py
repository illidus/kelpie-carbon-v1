...
class TestAsyncErrorHandling:
    ...
    @pytest.mark.parametrize("test_case", [
        # Real world validation error handling
        {
            ...
            "function_args": [
                ValidationSite(
                    name="Test Site", lat=50.0, lng=-125.0, species="Test",
        ...
        # Environmental testing error handling
        {
            ...
            "function_args": [
                EnvironmentalCondition(
                    name="test_condition", description="Test condition",
                    parameters={}, expected_behavior="Test"
                ),
                50.0, -126.0, "2023-07-01", "2023-07-31"
            ],
            "expected_success": False,
    ...
    async def test_async_error_handling(self, test_case):
        ...
        with patch(test_case["mock_target"]) as mock_func:
            ...
            result = await getattr(instance, test_case["async_function"])(*test_case["function_args"])
            ...
            if hasattr(result, 'error_message'):
                assert result.error_message is not None  # Just verify error was captured
            elif hasattr(result, 'metadata') and 'error' in result.metadata:
                assert result.metadata["error"] is not None  # Just verify error was captured
...
class TestSyncErrorHandling:
    ...
    @pytest.mark.parametrize("test_case", [
        # Species classification error handling
        {
            ...
            "function_args": [None, None, None],  # rgb_image, spectral_indices, kelp_mask
            "expected_result": {"primary_species": KelpSpecies.UNKNOWN, "confidence": 0.0},
    ...
    def test_sync_error_handling(self, test_case):
        ...
        if test_case["mock_target"]:
            with patch.object(instance, test_case["mock_target"], side_effect=test_case["mock_exception"]):
                result = getattr(instance, test_case["function"])(*test_case["function_args"])
        else:
            result = getattr(instance, test_case["function"])(*test_case["function_args"])
