        ...
        [
            ...
            # Valid EnvironmentalCondition creation
            {
                ...
                "params": {
                    ...
                    "parameters": {"tidal_height": 0.5, "correction_factor": -0.225},
    ...
    def test_valid_data_structure_creation(self, test_case):
        ...
        for assertion, expected_value in test_case["assertions"].items():
            if "isinstance(" in assertion:
                ...
                type_check = (
                    assertion.split("isinstance(")[1].split(")")[0].split(", ")[1]
                )
...
class TestContainerStructureValidation:
    ...
    @pytest.mark.parametrize(
        ...
        [
            # Dictionary structure validation
            {
                ...
                "validation_func": lambda data: all(
                    isinstance(year, int) for year in data.keys()
                ),
                "expected": True,
            ...
            },
            {
                ...
                "validation_func": lambda data: all(
                    isinstance(value, dict) and "extent" in value
            ...
            },
            {
                ...
                "validation_func": lambda data: isinstance(data, dict)
                and len(data) > 0,
                "expected": True,
        ...
        [
            # List structure validation
            {
                ...
                "validations": [
                    ...
                    (
                        "no_empty_strings",
                        lambda d: all(len(item.strip()) > 0 for item in d),
                    ),
                ],
            },
            {
            ...
            },
            {
                ...
                "validations": [
                    ...
                    (
                        "required_keys",
                        lambda d: all(
                            "site_id" in item and "lat" in item and "lng" in item
                            for item in d
                        ),
                    ),
                ],
    ...
    def test_list_structure_validation(self, test_case):
        ...
        for validation_name, validation_func in test_case["validations"]:
            ...
            assert result, (
                f"Failed {validation_name} validation for {test_case['name']}"
            )
        ...
        [
            # Complex nested structure validation
            {
                ...
                "validations": [
                    (
                        "has_metadata",
                        lambda d: "report_metadata" in d
                        and isinstance(d["report_metadata"], dict),
                    ),
                    (
                        "has_overall_quality",
                        lambda d: "overall_quality" in d
                        and "score" in d["overall_quality"],
                    ),
                    (
                        "has_site_quality",
                        lambda d: "site_quality" in d
                        and isinstance(d["site_quality"], dict),
                    ),
                    (
                        "has_recommendations",
                        lambda d: "recommendations" in d
                        and isinstance(d["recommendations"], list),
                    ),
                    (
                        "valid_scores",
                        lambda d: all(
                            0.0 <= site["quality_score"] <= 1.0
                            for site in d["site_quality"].values()
                        ),
                    ),
                ],
            },
            {
                ...
                "data": {
                    ...
                    "biomass_datasets": {
                        "BC_VALIDATION": {
                            "measurements": [10.5, 11.2, 9.8],
                            "units": "kg/m2",
                        },
                        "CA_VALIDATION": {
                            "measurements": [8.3, 9.1, 7.9],
                            "units": "kg/m2",
                        },
                    },
                    "validation_results": {
                ...
                "validations": [
                    (
                        "has_sites",
                        lambda d: "validation_sites" in d
                        and len(d["validation_sites"]) > 0,
                    ),
                    (
                        "has_datasets",
                        lambda d: "biomass_datasets" in d
                        and isinstance(d["biomass_datasets"], dict),
                    ),
                    (
                        "has_results",
                        lambda d: "validation_results" in d
                        and "overall_accuracy" in d["validation_results"],
                    ),
                    (
                        "site_consistency",
                        lambda d: set(site["site_id"] for site in d["validation_sites"])
                        == set(d["biomass_datasets"].keys()),
                    ),
                    (
                        "valid_measurements",
                        lambda d: all(
                            isinstance(dataset["measurements"], list)
                            and len(dataset["measurements"]) > 0
