...
class TestImageOptimization:
    ...
    def test_image_optimization_file_size(self):
        ...
        assert png_response.headers["Content-Length"] != jpeg_response.headers["Content-Length"]
...
class TestProgressiveLoading:
    ...
    def test_layer_priority_order(self):
        ...
        priority_1_layers = [layer for layer, priority in layer_priorities if priority == 1]
        priority_2_layers = [layer for layer, priority in layer_priorities if priority == 2]
        priority_3_layers = [layer for layer, priority in layer_priorities if priority == 3]
        ...
        def end_loading(operation_id, success=True):
            if operation_id in loading_states:
                loading_states[operation_id].update({
                    ...
                    'end_time': time.time(),
                    'duration': time.time() - loading_states[operation_id]['start_time']
                })
...
class TestPerformanceMonitoring:
    ...
    def test_cache_efficiency_calculation(self):
        ...
        relevant_loads = [load for load in image_loads if load['operation'] == 'image_load']
