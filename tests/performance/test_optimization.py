...
class TestCacheManagement:
    ...
    def test_cache_access_time_tracking(self):
        ...
        mock_dataset = xr.Dataset({"red": (["y", "x"], np.random.rand(5, 5))})
    ...
    def test_cache_cleanup_by_count(self):
        ...
        for i in range(Processing.MAX_CACHE_ITEMS + 5):
            mock_dataset = xr.Dataset({"data": (["y", "x"], np.random.rand(2, 2))})
    ...
    def test_cache_lru_eviction(self):
        ...
        datasets = []
        for i in range(5):
            mock_dataset = xr.Dataset({"data": (["y", "x"], np.random.rand(3, 3))})
...
class TestPystacClientFix:
    ...
    def test_items_method_usage(self):
        ...
        with patch("kelpie_carbon_v1.core.fetch.Client") as mock_client:
            ...
            assert (
                not hasattr(mock_search, "get_items")
                or not mock_search.get_items.called
            )
