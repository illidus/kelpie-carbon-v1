...
class TestCompleteWorkflow:
    ...
    def test_phase2_spectral_visualizations(self, mock_fetch):
        ...
        response = self.client.post("/api/imagery/analyze-and-cache", json={
            "aoi": {"lat": self.test_coordinates["lat"], "lng": self.test_coordinates["lng"]},
            **self.test_date_range
        })
...
class TestAPIEndpointIntegration:
    ...
    def test_api_error_handling(self):
        ...
        response = self.client.post("/api/run", json={
            "aoi": {"lat": 34.4140}  # Missing longitude
        })
        assert response.status_code == 422
...
class TestDataProcessingIntegration:
    ...
    def test_image_generation_pipeline(self, mock_fetch):
        ...
        # Generate imagery
        response = client.post("/api/imagery/analyze-and-cache", json={
            "aoi": {"lat": 34.4140, "lng": -119.8489},
            "start_date": "2023-06-01",
            "end_date": "2023-08-31"
        })
    ...
    def _create_realistic_mock_dataset(self):
        ...
        red_values = np.where(water_mask, 
                             np.random.rand(*size) * 0.05 + 0.02,  # Low water reflectance
                             np.random.rand(*size) * 0.3 + 0.1)    # Higher land reflectance
        red_values = np.where(kelp_mask, red_values * 0.5, red_values)  # Kelp absorbs red
        
        # NIR band (higher for vegetation, including kelp)
        nir_values = np.where(water_mask,
                             np.random.rand(*size) * 0.02 + 0.01,  # Very low water NIR
                             np.random.rand(*size) * 0.5 + 0.3)    # Higher land NIR
        nir_values = np.where(kelp_mask, 
                             np.random.rand(*size) * 0.3 + 0.2, 
                             nir_values)  # Moderate kelp NIR
        ...
        mock_dataset.red_edge = Mock()
        mock_dataset.red_edge.values = ((red_values + nir_values) / 2).astype(np.float32)
