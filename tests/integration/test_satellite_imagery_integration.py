...
class TestSatelliteImageryIntegration:
    ...
    def test_complete_imagery_pipeline(self, mock_predict, mock_mask, mock_indices, mock_fetch):
        ...
        false_color_response = self.client.get(f"/api/imagery/{analysis_id}/false-color")
        ...
        for index_name in spectral_indices:
            spectral_response = self.client.get(f"/api/imagery/{analysis_id}/spectral/{index_name}")
            ...
            print(f"✅ {index_name.upper()} visualization generated: {spectral_image.size}")
        ...
        for mask_type in mask_types:
            mask_response = self.client.get(f"/api/imagery/{analysis_id}/mask/{mask_type}")
        ...
        custom_mask_response = self.client.get(f"/api/imagery/{analysis_id}/mask/kelp?alpha=0.8")
    ...
    def test_imagery_performance(self):
        ...
        _analysis_cache[analysis_id] = {
            ...
            'indices': {'ndvi': (large_dataset['nir'] - large_dataset['red']) / 
                               (large_dataset['nir'] + large_dataset['red'])},
            'masks': {'kelp_mask': xr.DataArray(
                np.random.choice([True, False], (200, 250)),
                dims=['y', 'x']
            )},
            'biomass': xr.DataArray(
                np.random.rand(200, 250) * 5000,
                dims=['y', 'x']
            )
        }
    ...
    def test_layer_name_mapping_functionality(self):
        ...
        for internal_name, expected_display in internal_to_display.items():
            ...
            assert display_name == expected_display, f"Layer name mapping failed for {internal_name}"
