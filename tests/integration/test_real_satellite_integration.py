...
from kelpie_carbon_v1.data.skema_integration import (
    SKEMADataIntegrator,
...
class TestRealSatelliteDataIntegration:
    ...
    def fetch_real_training_data(
        self, locations: List[dict], date_range: dict, max_samples: int = 4
    ) -> List[Tuple[xr.Dataset, float]]:
        ...
        for i, location in enumerate(locations[:max_samples]):
            try:
                ...
                print(
                    f"Fetched data for location {i+1}: "
    ...
    def _estimate_biomass_from_spectral_data(
        self, dataset: xr.Dataset, expected_kelp: bool
    ) -> float:
        ...
        if expected_kelp:
            ...
            estimated_biomass = min(
                estimated_biomass, 500
            )  # Maximum 500 kg/ha for non-kelp areas
    ...
    def test_real_data_model_training(self, kelp_forest_locations, date_range):
        ...
        assert (
            metrics["n_features"] > 30
        ), "Should extract substantial features from real data"
        ...
        assert (
            metrics["test_r2"] >= -10
        ), "Test R² should be reasonable for mixed data sources"
    ...
    def test_real_data_prediction_pipeline(self, kelp_forest_locations, date_range):
        ...
        try:
            ...
            if test_location["expected_kelp"]:
                print(
                    f"Kelp forest location biomass: {prediction['biomass_kg_per_hectare']:.1f} kg/ha"
                )
    ...
    def test_real_vs_synthetic_model_comparison(
        self, kelp_forest_locations, date_range
    ):
        ...
        try:
            ...
            print(
                f"Real model prediction: {real_prediction['biomass_kg_per_hectare']:.1f} kg/ha"
            )
            print(
                f"Synthetic model prediction: {synthetic_prediction['biomass_kg_per_hectare']:.1f} kg/ha"
            )
            print(
                f"Real model confidence: {real_prediction['prediction_confidence']:.3f}"
            )
            print(
                f"Synthetic model confidence: {synthetic_prediction['prediction_confidence']:.3f}"
            )
            ...
            assert (
                synthetic_prediction["model_type"] == "Synthetic (Spectral Index Based)"
            )
    ...
    def test_real_data_feature_extraction(self, kelp_forest_locations, date_range):
        ...
        try:
            ...
            print(
                f"Extracted {len(features.columns)} features from real satellite data:"
            )
            ...
            assert (
                len(features.columns) >= 30
            ), "Should extract substantial number of features"

            # Check for key feature categories
            feature_names = features.columns.tolist()
            assert any(
                "kelp" in name for name in feature_names
            ), "Should have kelp-related features"
            assert any(
                "fai" in name for name in feature_names
            ), "Should have FAI features"
            assert any(
                "ndre" in name for name in feature_names
            ), "Should have NDRE features"
            assert any(
                "water" in name for name in feature_names
    ...
    def test_model_persistence_with_real_data(
        self, kelp_forest_locations, date_range, tmp_path
    ):
    ...
    def test_skema_integration(self):
        ...
        print(
            f"✓ Successfully integrated {len(positive_samples)} positive "
        ...
        assert (
            len(high_confidence_points) >= 1
        ), "Should have high-confidence validation points"

        # Test species information
        integrator = SKEMADataIntegrator()
        species_info = integrator.get_species_info()
        assert (
            "Macrocystis pyrifera" in species_info
        ), "Should have Giant Kelp species info"
        assert (
            "Nereocystis luetkeana" in species_info
        ), "Should have Bull Kelp species info"

        print("✓ SKEMA integration test completed successfully")
        print(
            f"✓ Found {len(high_confidence_points)} high-confidence validation points"
        )
        ...
        assert (
            validation_results["validation_source"] == "skema_uvic"
        ), "Should indicate SKEMA source"

        print(
            f"✓ Model validation against SKEMA data: {validation_results['total_matches']} matches"
        )
...
def test_real_satellite_data_manual():
    ...
    try:
        ...
        if "data" in result:
            ...
            print(
                f"🌊 Biomass prediction: {prediction['biomass_kg_per_hectare']:.1f} kg/ha"
            )
