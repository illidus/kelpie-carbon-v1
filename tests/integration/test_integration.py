...
from kelpie_carbon_v1.core.mask import apply_mask, get_mask_statistics, create_water_mask, create_kelp_detection_mask, calculate_fai, calculate_red_edge_ndvi
...
def test_water_mask_debug():
    ...
    print(f"  Red Edge: {red_edge.min():.4f} - {red_edge.max():.4f} (mean: {red_edge.mean():.4f})")
...
def test_indices_calculation_with_realistic_data(realistic_kelp_dataset):
    ...
    for index_name in expected_indices:
        ...
        print(f"{index_name}: mean={mean_val:.4f}, min={np.nanmin(index_values):.4f}, max={np.nanmax(index_values):.4f}")
        
        # Specific range checks for indices
        if index_name in ["ndre"]:
            assert 0 <= mean_val <= 1, f"{index_name} should be between 0 and 1, got {mean_val}"
...
def test_masking_with_realistic_data(realistic_kelp_dataset):
    ...
    assert mask_stats["valid_coverage_percent"] > 50, "Expected reasonable valid coverage"
...
def test_biomass_prediction_with_realistic_data(realistic_kelp_dataset):
    ...
    required_fields = ["biomass_kg_per_hectare", "prediction_confidence", "top_features", "model_type"]
    ...
    assert isinstance(biomass, (int, float)), f"Biomass should be numeric, got {type(biomass)}"
...
def test_empty_data_handling():
    ...
    print(f"Indices with zero data: {[f'{k}: {np.nanmean(v.values):.4f}' for k, v in indices.data_vars.items()]}")
    ...
    # Test prediction
    result = predict_biomass(masked_data)
    print(f"Prediction with zero data: biomass={result['biomass_kg_per_hectare']}, confidence={result['prediction_confidence']}")
...
def test_pipeline_debug_values():
    ...
    red_edge[kelp_area] = np.random.uniform(0.08, 0.15, np.sum(kelp_area))  # High red edge
    ...
    for var_name, var_data in indices.data_vars.items():
        ...
        print(f"  {var_name}: mean={mean_val:.4f}, std={std_val:.4f}, range=[{min_val:.4f}, {max_val:.4f}]")
...
def test_api_with_mock_data(test_client):
    ...
    try:
        ...
        if data["status"] == "completed" and "Error" not in data["biomass"]:
            ...
            assert biomass_value >= 0, f"Expected non-negative biomass, got {biomass_value}"
