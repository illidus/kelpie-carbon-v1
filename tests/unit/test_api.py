...
def test_run_analysis_endpoint(test_client, sample_coordinates):
    ...
    assert result["status"] in ["completed", "error"]  # Allow for both success and error
...
def test_run_analysis_endpoint_invalid_coordinates(test_client, invalid_coordinates):
    ...
    for invalid_coord in invalid_coordinates:
        if isinstance(invalid_coord["lat"], (int, float)) and isinstance(invalid_coord["lng"], (int, float)):
            test_request = {
                "aoi": invalid_coord,
                "start_date": "2023-01-01",
                "end_date": "2023-01-31",
