...
def test_mock_validation_generator(tmp_path):
    ...
    try:
        ...
        assert len(kelp_with_spectra) >= len(
            kelp_measurements
        )  # All kelp should have spectral data
    finally:
        # Clean up
        data_manager.close()
        import gc
...
def test_integration_workflow(tmp_path):
    ...
    try:
        ...
        ndre_predictions = [
            gt if i % 5 != 0 else not gt for i, gt in enumerate(ground_truth)
        ]
        ndvi_predictions = [
            gt if i % 6 != 0 else not gt for i, gt in enumerate(ground_truth)
        ]
