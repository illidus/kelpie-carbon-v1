...
class TestImageryUtils:
    ...
    def test_normalize_to_0_1(self):
        ...
        data = xr.DataArray(
            np.random.rand(30, 30) * 100 - 50,  # Range -50 to 50
            dims=['y', 'x']
        )
    ...
    def test_get_image_bounds(self):
        ...
        dataset = xr.Dataset({
            ...
            'x': np.linspace(-122, -121, 30),
            'y': np.linspace(36, 37, 20)
        })
...
class TestImageGenerators:
    ...
    def setup_method(self):
        ...
        self.dataset = xr.Dataset({
            ...
            'x': np.linspace(-122, -121, 60),
            'y': np.linspace(36, 37, 50)
        })
    ...
    def test_generate_spectral_visualization(self):
        ...
        ndvi = (self.dataset['nir'] - self.dataset['red']) / (self.dataset['nir'] + self.dataset['red'])
...
class TestOverlayGenerators:
    ...
    def setup_method(self):
        ...
        self.dataset = xr.Dataset({
            'kelp_mask': (['y', 'x'], np.random.choice([True, False], (40, 50), p=[0.3, 0.7])),
            'water_mask': (['y', 'x'], np.random.choice([True, False], (40, 50), p=[0.6, 0.4])),
            'cloud_mask': (['y', 'x'], np.random.choice([True, False], (40, 50), p=[0.1, 0.9])),
        })
    ...
    def test_generate_biomass_heatmap(self):
        ...
        biomass_data = xr.DataArray(
            np.random.rand(35, 45) * 5000,  # Biomass in kg/ha
            dims=['y', 'x']
        )
...
class TestImageryIntegration:
    ...
    def test_full_imagery_pipeline(self):
        ...
        fai = dataset['nir'] - (dataset['red'] + (dataset['red_edge'] - dataset['red']) * 
                               ((865 - 665) / (705 - 665)))
    ...
    def test_imagery_with_real_satellite_data_structure(self):
        ...
        dataset = xr.Dataset({
            'red': (['y', 'x'], np.random.rand(100, 120) * 0.3),  # Reflectance values
    ...
    def test_error_handling(self):
        ...
        wrong_dataset = xr.Dataset({
            'wrong_band': (['y', 'x'], np.random.rand(10, 10))
        })
