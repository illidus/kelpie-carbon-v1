        ...
        if expected_error:
            ...
            if metric_type == "detection_accuracy" and input_data:
                result = calculator.calculate_detection_metrics(
                    predicted=input_data, ground_truth=[0.6, 0.7, 0.8]
                )
...
def test_analysis_creation_scenarios(
    analysis_type, site_data, time_data, should_succeed
):
    ...
    try:
        ...
        analysis_types = [
            (
                ...
                else AnalysisType.TEMPORAL
            )
        ]
        coordinates = (site_data["lat"], site_data["lon"])
        time_range = (
            datetime.fromisoformat(time_data["start"]),
            datetime.fromisoformat(time_data["end"]),
        )
