    ...
    def analyze_validation_patterns(self):
        ...
        for test_file in self.test_root.rglob("test_*.py"):
            ...
            try:
                ...
                if re.search(r'invalid.*latitude|latitude.*invalid', content, re.IGNORECASE):
                    patterns['coordinate_validation'].append(test_file)
                    
                if re.search(r'invalid.*longitude|longitude.*invalid', content, re.IGNORECASE):
                    patterns['coordinate_validation'].append(test_file)
                ...
                if re.search(r'test.*validation|validation.*test', content, re.IGNORECASE):
...
'''
    ...
    def identify_duplicate_files(self):
        ...
        for test_file in self.test_root.rglob("test_*.py"):
            ...
            try:
                ...
                validation_tests = len(re.findall(r'def test.*validation', content, re.IGNORECASE))
                validation_tests += len(re.findall(r'def test.*invalid', content, re.IGNORECASE))
                validation_tests += len(re.findall(r'def test.*error', content, re.IGNORECASE))
                
                total_tests = len(re.findall(r'def test_', content))
                
                if total_tests > 0 and validation_tests / total_tests > 0.7:
                    validation_heavy_files.append((test_file, validation_tests, total_tests))
    ...
    def remove_duplicate_tests(self):
        ...
        for test_file, validation_count, total_count in self.duplicate_files:
            if validation_count >= 5:  # Only remove if significant number of validations
                try:
                    with open(test_file, 'r', encoding='utf-8') as f:
                        content = f.read()
...
def main():
    ...
    print(f"\n🔄 Next steps:")
    print(f"   1. Run: pytest tests/param/ -v")
    print(f"   2. Run: pytest --cov=src --cov-report=term-missing")
    print(f"   3. Compare test counts and coverage")
